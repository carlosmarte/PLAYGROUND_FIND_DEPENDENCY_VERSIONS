...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.PacmanVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.PacmanVersionsSDK`` directly.
...
exits:
    python3 cli.py versions bash
    python3 cli.py run bash --limit 5 --first-only -v
...
class PacmanVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.PacmanVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"repository = {self.sdk.effective_index_url() or '(pacman default)'}")
    ...
    def do_pacman(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.pacman_version = arg
            self.sdk.invalidate_venv()  # re-check on next install-test
        print(f"pacman-version = {self.cfg.pacman_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full pacman output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams pacman's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output     = {self.cfg.output}")
        print(f"  venv-dir   = {self.cfg.venv_dir}")
        print(f"  pacman     = {self.cfg.pacman_version}")
        print(f"  verbose    = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved pacman/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.PacmanVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test bash         # every version the repository advertises
            test bash 10      # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run bash --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <package> [--repository URL] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

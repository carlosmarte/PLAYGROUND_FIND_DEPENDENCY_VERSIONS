...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  PacmanVersionsSDK  ->  main.py
    external  ->  PacmanVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``PacmanVersionsSDK`` and override the
...
External one-liners::
    ...
    import sdk
    report = sdk.test("bash", index_url="https://archive.archlinux.org", limit=5)
    print(report.installable)          # ['5.2.026-1', ...]
...
Object form (hold one per session, mutate ``.config`` freely)::
    ...
    s = sdk.PacmanVersionsSDK(index_url="https://archive.archlinux.org", pacman_version="6.1.0")
    s.config.package = "bash"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.PacmanVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.PacmanVersionsSDK().run(["bash", "--limit", "5", "--first-only"])
"""
...
DEFAULT_PACMAN_VERSION = main.DEFAULT_PACMAN_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use pacman's default, no
# --repository; limit=None => test every version).
_UNSET = object()


class PacmanVersionsError(RuntimeError):
...
class Config:
    ...
    ``$PACMAN_REPOSITORY > $PACMAN_REGISTRY_URL > Arch archive`` chain. An
    explicit ``None`` means "use pacman's own default" (omit ``--repository``
    entirely).
    """

    package: Optional[str] = None
    index_url: object = _UNSET
    venv_dir: str = ".pacman-test-install"
    output: str = "installation_report.json"
    pacman_version: Optional[str] = DEFAULT_PACMAN_VERSION  # None/"none" => skip the version check
    limit: Optional[int] = None
    verbose: bool = False  # stream full pacman output so installs are debuggable
...
class PacmanVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._cache_path = None  # lazily provisioned test cache dir

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "PacmanVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--repository`` resolves from the
        environment (``_UNSET``), and ``--pacman-version none`` skips the host
        pacman version check.
        """
        ns = main.parse_args(argv)
        pacman_version = None if str(ns.pacman_version).lower() == "none" else ns.pacman_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The repo URL pacman will actually receive (``None`` => pacman default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.pacman_version``.
        """
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise PacmanVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise PacmanVersionsError(f"no versions found for {pkg!r}")

...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  CondaVersionsSDK  ->  main.py
    external  ->  CondaVersionsSDK  ->  main.py
...
results (``Report``), and is built to be:
  ...
  * **extended** — subclass ``CondaVersionsSDK`` and override the
...
External one-liners::
    ...
    import sdk
    report = sdk.test("numpy", index_url="conda-forge", limit=5)
    print(report.installable)          # ['2.1.3', ...]

    versions = sdk.versions("numpy")   # just list what the channel advertises
...
Object form (hold one per session, mutate ``.config`` freely)::
    ...
    s = sdk.CondaVersionsSDK(index_url="conda-forge", conda_version="24.9.2")
    s.config.package = "numpy"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.CondaVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.CondaVersionsSDK().run(["numpy", "--limit", "5", "--first-only"])
"""
...
DEFAULT_CONDA_VERSION = main.DEFAULT_CONDA_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use conda's default channels, no
# override; limit=None => test every version).
_UNSET = object()


class CondaVersionsError(RuntimeError):
...
class Config:
    ...
    ``None`` means "use conda's own default channels" (omit the override).
    """

    package: Optional[str] = None
    index_url: object = _UNSET
    venv_dir: str = ".conda-test-install"
    output: str = "installation_report.json"
    conda_version: Optional[str] = DEFAULT_CONDA_VERSION  # None/"none" => skip the check
    limit: Optional[int] = None
    verbose: bool = False  # stream full conda output so installs are debuggable
...
class CondaVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._prefix_root = None  # lazily provisioned test-prefix root

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "CondaVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--channel`` resolves from the
        environment (``_UNSET``), and ``--conda-version none`` skips the conda
        version confirmation step.
        """
        ns = main.parse_args(argv)
        conda_version = None if str(ns.conda_version).lower() == "none" else ns.conda_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The channel conda will actually receive (``None`` => conda default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.conda_version``.
        """
    ...
    def ensure_pip(self) -> str:
        ...
        if self._prefix_root is None:
            cv = self.config.conda_version
            cv = None if (cv is None or str(cv).lower() == "none") else cv
            self._prefix_root = main.setup_venv(
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise CondaVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise CondaVersionsError(f"no versions found for {pkg!r}")

        prefix_root = self.ensure_pip()
        results = main.test_installations(
            prefix_root, pkg, index_url, versions, self.config.output,
...
def versions(package, **config) -> List[str]:
    """One-shot: list versions a channel advertises for ``package``."""
    return CondaVersionsSDK(package=package, **config).available_versions()


def versions_output(package, **config) -> dict:
    """One-shot: JSON-able envelope of the versions a registry advertises."""
    return CondaVersionsSDK(package=package, **config).versions_output()


def find(package, **config) -> Report:
    """One-shot: install-test until the first version that works."""
    return CondaVersionsSDK(package=package, **config).find()


def test(package, **config) -> Report:
    """One-shot: install-test versions and write a report."""
    return CondaVersionsSDK(package=package, **config).test()


def run(argv) -> int:
    """One-shot raw passthrough to ``main.main``."""
    return CondaVersionsSDK().run(argv)

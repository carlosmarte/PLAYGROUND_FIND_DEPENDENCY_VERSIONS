...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.CondaVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.CondaVersionsSDK`` directly.
...
class CondaVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.CondaVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"channel = {self.sdk.effective_index_url() or '(conda default)'}")
    ...
    def do_venv(self, arg):
        ...
        print(f"prefix-root = {self.cfg.venv_dir}")

    def do_conda(self, arg):
        """conda [VERSION|none]  — set or show the conda version expected in the test env."""
        arg = arg.strip()
        if arg:
            self.cfg.conda_version = arg
            self.sdk.invalidate_venv()  # re-check on next install-test
        print(f"conda-version = {self.cfg.conda_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full conda output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams conda's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output      = {self.cfg.output}")
        print(f"  prefix-root = {self.cfg.venv_dir}")
        print(f"  conda       = {self.cfg.conda_version}")
        print(f"  verbose     = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved conda/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.CondaVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test numpy        # every version the channel advertises
    ...
    def do_run(self, arg):
        ...
        if not argv:
            print("Usage: run <package> [--channel CHANNEL] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

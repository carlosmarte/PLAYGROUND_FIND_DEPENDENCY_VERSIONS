...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  CabalVersionsSDK  ->  main.py
    external  ->  CabalVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``CabalVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    report = sdk.find("aeson")         # stop at the first version that fetches

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("aeson")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("aeson")       # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.CabalVersionsSDK(index_url="https://hackage.haskell.org", cabal_version="3.12.1.0")
    s.config.package = "aeson"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.CabalVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.CabalVersionsSDK().run(["aeson", "--limit", "5", "--first-only"])
"""
...
DEFAULT_CABAL_VERSION = main.DEFAULT_CABAL_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use cabal's default, no
# --index-url; limit=None => test every version).
_UNSET = object()


class CabalVersionsError(RuntimeError):
...
class Config:
    ...
    ``None`` means "use cabal's own default" (omit ``--index-url`` entirely).
    """

    package: Optional[str] = None
    index_url: object = _UNSET
    venv_dir: str = ".cabal-test-fetch"
    output: str = "installation_report.json"
    cabal_version: Optional[str] = DEFAULT_CABAL_VERSION  # None/"none" => skip version check
    limit: Optional[int] = None
    verbose: bool = False  # stream full cabal output so fetches are debuggable
...
class CabalVersionsSDK:
    ...
    @classmethod
    def from_argv(cls, argv) -> "CabalVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--index-url`` resolves from the
        environment (``_UNSET``), and ``--cabal-version none`` skips the sandbox
        version check.
        """
        ns = main.parse_args(argv)
        cabal_version = None if str(ns.cabal_version).lower() == "none" else ns.cabal_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The index URL cabal will actually receive (``None`` => cabal default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.cabal_version``.
        """
        self._sandbox = None
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise CabalVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise CabalVersionsError(f"no versions found for {pkg!r}")

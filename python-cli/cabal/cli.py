...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.CabalVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.CabalVersionsSDK`` directly.
...
exits:
    python3 cli.py versions aeson
    python3 cli.py run aeson --limit 5 --first-only -v
...
class CabalVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.CabalVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"index-url = {self.sdk.effective_index_url() or '(cabal default)'}")
    ...
    def do_cabal(self, arg):
        """cabal [VERSION|none]  — set or show the cabal version expected in the sandbox."""
        arg = arg.strip()
        if arg:
            self.cfg.cabal_version = arg
            self.sdk.invalidate_venv()  # re-check on next fetch-test
        print(f"cabal-version = {self.cfg.cabal_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full cabal output so fetches are debuggable.

        With no argument, toggles. When on, every find/test streams cabal's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  cabal     = {self.cfg.cabal_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved cabal/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.CabalVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test aeson        # every version the registry advertises
            test aeson 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run aeson --limit 5 --first-only
        """

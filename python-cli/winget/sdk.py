...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  WingetVersionsSDK  ->  main.py
    external  ->  WingetVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``WingetVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("Git.Git", source="winget", limit=5)
    print(report.installable)          # ['2.45.0', ...]
...
Extension example::
    ...
    class QuietSDK(sdk.WingetVersionsSDK):
...
DEFAULT_WINGET_VERSION = main.DEFAULT_WINGET_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use winget's default, no
# --source; limit=None => test every version).
_UNSET = object()


class WingetVersionsError(RuntimeError):
...
class Config:
    ...
    ``$WINGET_SOURCE > $WINGET_REGISTRY_URL > winget`` chain. An explicit
    ``None`` means "use winget's own default" (omit ``--source`` entirely).
    """
    ...
    winget_version: Optional[str] = DEFAULT_WINGET_VERSION  # None/"none" => skip toolchain check
    limit: Optional[int] = None
    verbose: bool = False  # stream full winget output so installs are debuggable
...
class WingetVersionsSDK:
    ...
    @classmethod
    def from_argv(cls, argv) -> "WingetVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--source`` resolves from the
        environment (``_UNSET``), and ``--winget-version none`` skips the
        toolchain check.
        """
        ns = main.parse_args(argv)
        winget_version = None if str(ns.winget_version).lower() == "none" else ns.winget_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The source winget will actually receive (``None`` => its default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.winget_version``.
        """
        self._venv_dir = None
    ...
    def ensure_pip(self) -> str:
        ...
        if self._venv_dir is None:
            wv = self.config.winget_version
            wv = None if (wv is None or str(wv).lower() == "none") else wv
            self._venv_dir = main.setup_venv(
                self.config.venv_dir, wv, self.resolve_env(), verbose=self.config.verbose
            )
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise WingetVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise WingetVersionsError(f"no versions found for {pkg!r}")
...
def find(package, **config) -> Report:
    ...
    return WingetVersionsSDK(package=package, **config).find()


def test(package, **config) -> Report:
    """One-shot: install-test versions and write a report."""
    return WingetVersionsSDK(package=package, **config).test()


def run(argv) -> int:
    """One-shot raw passthrough to ``main.main``."""
    return WingetVersionsSDK().run(argv)

...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.WingetVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.WingetVersionsSDK`` directly.
...
exits:
    python3 cli.py versions Git.Git
    python3 cli.py run Git.Git --limit 5 --first-only -v
...
class WingetVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.WingetVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"source = {self.sdk.effective_index_url() or '(winget default)'}")
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  winget    = {self.cfg.winget_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved winget/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        except sdk.WingetVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test Git.Git        # every version the source advertises
            test Git.Git 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run Git.Git --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <package> [--source NAME] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

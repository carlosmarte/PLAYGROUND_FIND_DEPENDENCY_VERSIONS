...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.VagrantVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.VagrantVersionsSDK`` directly.
...
exits:
    python3 cli.py versions hashicorp/bionic64
    python3 cli.py run hashicorp/bionic64 --limit 5 --first-only -v
...
class VagrantVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.VagrantVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.vagrant_server = None if arg.lower() == "none" else arg
        print(f"vagrant-server = {self.sdk.effective_vagrant_server() or '(vagrant default)'}")
    ...
    def do_vagrant(self, arg):
        """vagrant [VERSION|none]  — set or show the vagrant version expected in the test env."""
        arg = arg.strip()
        if arg:
            self.cfg.vagrant_version = arg
            self.sdk.invalidate_venv()  # re-check on next install-test
        print(f"vagrant-version = {self.cfg.vagrant_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full vagrant output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams vagrant's
        live output (and a copy lands in the report) so you can see why a version
        failed to install.
        """
        arg = arg.strip().lower()
    ...
    def do_show(self, arg):
        ...
        print(f"  output         = {self.cfg.output}")
        print(f"  venv-dir       = {self.cfg.venv_dir}")
        print(f"  vagrant        = {self.cfg.vagrant_version}")
        print(f"  verbose        = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved vagrant/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        except sdk.VagrantVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test hashicorp/bionic64      # every version the Cloud advertises
            test hashicorp/bionic64 10   # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run hashicorp/bionic64 --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <package> [--vagrant-server URL] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

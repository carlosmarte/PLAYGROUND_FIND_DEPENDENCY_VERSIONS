...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  VagrantVersionsSDK  ->  main.py
    external  ->  VagrantVersionsSDK  ->  main.py
...
External one-liners::
    ...
    import sdk
    report = sdk.test("hashicorp/bionic64", vagrant_server="https://app.vagrantup.com", limit=5)
    print(report.installable)          # ['1.0.282', ...]
...
Extension example::
    ...
    class QuietSDK(sdk.VagrantVersionsSDK):
...
DEFAULT_VAGRANT_VERSION = main.DEFAULT_VAGRANT_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (vagrant_server=None => use vagrant's default;
# limit=None => test every version).
_UNSET = object()


class VagrantVersionsError(RuntimeError):
...
class Config:
    ...
    ``vagrant_server`` carries a third state beyond str/None: the ``_UNSET``
    sentinel means "resolve from the environment" (the default), matching
    ``main``'s ``$VAGRANT_SERVER_URL > $VAGRANT_REGISTRY_URL > app.vagrantup.com``
    chain. An explicit ``None`` means "use vagrant's own default" (omit the
    server override entirely).
    """

    package: Optional[str] = None
    vagrant_server: object = _UNSET
    venv_dir: str = ".venv-test-install"
    output: str = "installation_report.json"
    vagrant_version: Optional[str] = DEFAULT_VAGRANT_VERSION  # None/"none" => whatever is on PATH
    limit: Optional[int] = None
    verbose: bool = False  # stream full vagrant output so installs are debuggable
...
class VagrantVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._vagrant_home = None  # lazily provisioned sandbox VAGRANT_HOME

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "VagrantVersionsSDK":
        ...
        environment (``_UNSET``), and ``--vagrant-version none`` uses whatever
        vagrant is on PATH.
        """
        ns = main.parse_args(argv)
        vagrant_version = None if str(ns.vagrant_version).lower() == "none" else ns.vagrant_version
        return cls(Config(
            package=ns.package,
            vagrant_server=_UNSET if ns.vagrant_server is None else ns.vagrant_server,
    ...
    def effective_vagrant_server(self) -> Optional[str]:
        """The Vagrant server vagrant will actually receive (``None`` => default)."""
        if self.config.vagrant_server is _UNSET:
            return main.resolve_vagrant_server(None, self.resolve_env())
        return self.config.vagrant_server
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.vagrant_version``.
        """
        self._vagrant_home = None

    def ensure_pip(self) -> str:
        """Provision the sandbox VAGRANT_HOME once and return its path."""
        if self._vagrant_home is None:
            vv = self.config.vagrant_version
            vv = None if (vv is None or str(vv).lower() == "none") else vv
            self._vagrant_home = main.setup_venv(
                self.config.venv_dir, vv, self.resolve_env(), verbose=self.config.verbose
            )
    ...
    def available_versions(self, package=None, limit=_UNSET) -> List[str]:
        ...
        versions = main.get_available_versions(
            pkg, self.effective_vagrant_server(), self.resolve_env(),
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise VagrantVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        versions = main.get_available_versions(pkg, vagrant_server, cfg, verbose=self.config.verbose)
        versions = self._apply_limit(versions, limit)
        versions = self.before_probe(pkg, versions)
        if not versions:
            raise VagrantVersionsError(f"no versions found for {pkg!r}")

        vagrant_home = self.ensure_pip()
        results = main.test_installations(
            vagrant_home, pkg, vagrant_server, versions, self.config.output,
            first_only=first_only, cfg=cfg, verbose=self.config.verbose,
        )
        report = Report(
            package=pkg, vagrant_server=vagrant_server,
...
def find(package, **config) -> Report:
    ...
    return VagrantVersionsSDK(package=package, **config).find()


def test(package, **config) -> Report:
    """One-shot: install-test versions and write a report."""
    return VagrantVersionsSDK(package=package, **config).test()


def run(argv) -> int:
    """One-shot raw passthrough to ``main.main``."""
    return VagrantVersionsSDK().run(argv)

...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.ApkVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.ApkVersionsSDK`` directly.
...
exits:
    python3 cli.py versions busybox
    python3 cli.py run busybox --limit 5 --first-only -v
...
class ApkVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.ApkVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"repository = {self.sdk.effective_index_url() or '(apk default)'}")
    ...
    def do_apk(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.apk_version = arg
            self.sdk.invalidate_venv()  # re-check on next install-test
        print(f"apk-version = {self.cfg.apk_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full apk output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams apk's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output     = {self.cfg.output}")
        print(f"  venv-dir   = {self.cfg.venv_dir}")
        print(f"  apk        = {self.cfg.apk_version}")
        print(f"  verbose    = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved apk/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.ApkVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test busybox      # every version the repository advertises
            test busybox 10   # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run busybox --limit 5 --first-only
        """

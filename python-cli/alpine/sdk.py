...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  ApkVersionsSDK  ->  main.py
    external  ->  ApkVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``ApkVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("busybox", index_url="https://dl-cdn.alpinelinux.org/alpine/latest-stable/main", limit=5)
    print(report.installable)          # ['1.36.1-r5', ...]

    versions = sdk.versions("busybox") # just list what the repository advertises
    report = sdk.find("busybox")       # stop at the first version that installs

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("busybox")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("busybox")     # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.ApkVersionsSDK(index_url="https://reg/main", apk_version="2.14.4")
    s.config.package = "busybox"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.ApkVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.ApkVersionsSDK().run(["busybox", "--limit", "5", "--first-only"])
"""
...
DEFAULT_APK_VERSION = main.DEFAULT_APK_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use apk's default, no
# --repository; limit=None => test every version).
_UNSET = object()


class ApkVersionsError(RuntimeError):
...
class Config:
    ...
    ``None`` means "use apk's own default" (omit ``--repository`` entirely).
    """

    package: Optional[str] = None
    index_url: object = _UNSET
    venv_dir: str = ".apk-test-install"
    output: str = "installation_report.json"
    apk_version: Optional[str] = DEFAULT_APK_VERSION  # None/"none" => skip the version check
    limit: Optional[int] = None
    verbose: bool = False  # stream full apk output so installs are debuggable
...
class ApkVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._root_path = None  # lazily provisioned test apk root

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "ApkVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--repository`` resolves from the
        environment (``_UNSET``), and ``--apk-version none`` skips the host apk
        version check.
        """
        ns = main.parse_args(argv)
        apk_version = None if str(ns.apk_version).lower() == "none" else ns.apk_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The repo URL apk will actually receive (``None`` => apk default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.apk_version``.
        """
    ...
    def ensure_pip(self) -> str:
        ...
        if self._root_path is None:
            av = self.config.apk_version
            av = None if (av is None or str(av).lower() == "none") else av
            self._root_path = main.setup_venv(
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise ApkVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise ApkVersionsError(f"no versions found for {pkg!r}")

        root_path = self.ensure_pip()
        results = main.test_installations(
            root_path, pkg, index_url, versions, self.config.output,

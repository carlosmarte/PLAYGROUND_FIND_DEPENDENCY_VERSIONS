...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  DartVersionsSDK  ->  main.py
    external  ->  DartVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``DartVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("http", index_url="https://pub.dev", limit=5)
    print(report.installable)          # ['1.2.2', ...]

    versions = sdk.versions("http")    # just list what the registry advertises
    report = sdk.find("http")          # stop at the first version that installs

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("http")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("http")        # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.DartVersionsSDK(index_url="https://pub.dev", dart_version="3.5.4")
    s.config.package = "http"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.DartVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.DartVersionsSDK().run(["http", "--limit", "5", "--first-only"])
"""
...
DEFAULT_DART_VERSION = main.DEFAULT_DART_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use pub's default hosted URL, no
# override; limit=None => test every version).
_UNSET = object()


class DartVersionsError(RuntimeError):
...
class Config:
    ...
    ``None`` means "use pub's own default hosted URL" (omit the override).
    """

    package: Optional[str] = None
    index_url: object = _UNSET
    venv_dir: str = ".dart-test-install"
    output: str = "installation_report.json"
    dart_version: Optional[str] = DEFAULT_DART_VERSION  # None/"none" => skip the check
    limit: Optional[int] = None
    verbose: bool = False  # stream full pub output so installs are debuggable
...
class DartVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._pkg_dir = None  # lazily provisioned test-package directory

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "DartVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--hosted-url`` resolves from the
        environment (``_UNSET``), and ``--dart-version none`` skips the dart
        version confirmation step.
        """
        ns = main.parse_args(argv)
        dart_version = None if str(ns.dart_version).lower() == "none" else ns.dart_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The hosted URL pub will actually receive (``None`` => pub default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.dart_version``.
        """
        self._pkg_dir = None
    ...
    def ensure_pip(self) -> str:
        ...
        if self._pkg_dir is None:
            dv = self.config.dart_version
            dv = None if (dv is None or str(dv).lower() == "none") else dv
            self._pkg_dir = main.setup_venv(
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise DartVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise DartVersionsError(f"no versions found for {pkg!r}")

        pkg_dir = self.ensure_pip()
        results = main.test_installations(
            pkg_dir, pkg, index_url, versions, self.config.output,

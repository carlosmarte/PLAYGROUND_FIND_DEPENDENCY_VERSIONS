...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  ChocoVersionsSDK  ->  main.py
    external  ->  ChocoVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``ChocoVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("git", source="https://community.chocolatey.org/api/v2/", limit=5)
    print(report.installable)          # ['2.47.1', ...]

    versions = sdk.versions("git")   # just list what the registry advertises
    report = sdk.find("git")         # stop at the first version that installs
...
Structured output (call from any consuming script — no console scraping)::
    ...
    sdk.versions_output("git")         # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.ChocoVersionsSDK(source="https://community.chocolatey.org/api/v2/", choco_version="2.3.0")
    s.config.package = "git"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.ChocoVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.ChocoVersionsSDK().run(["git", "--limit", "5", "--first-only"])
"""
...
DEFAULT_CHOCO_VERSION = main.DEFAULT_CHOCO_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use choco's default, no
# --source; limit=None => test every version).
_UNSET = object()


class ChocoVersionsError(RuntimeError):
...
class Config:
    ...
    explicit ``None`` means "use choco's own default" (omit ``--source``
    entirely).
    """
    ...
    choco_version: Optional[str] = DEFAULT_CHOCO_VERSION  # None/"none" => skip toolchain check
    limit: Optional[int] = None
    verbose: bool = False  # stream full choco output so installs are debuggable
...
class ChocoVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._venv_dir = None  # lazily provisioned test-sandbox dir

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "ChocoVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--source`` resolves from the
        environment (``_UNSET``), and ``--choco-version none`` skips the
        toolchain check.
        """
        ns = main.parse_args(argv)
        choco_version = None if str(ns.choco_version).lower() == "none" else ns.choco_version
        return cls(Config(
            package=ns.package,
            index_url=_UNSET if ns.source is None else ns.source,
            venv_dir=ns.venv_dir,
            output=ns.output,
            choco_version=choco_version,
            limit=ns.limit,
            verbose=ns.verbose,
        ))
    ...
    def effective_index_url(self) -> Optional[str]:
        """The source URL choco will actually receive (``None`` => its default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.choco_version``.
        """
        self._venv_dir = None
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise ChocoVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise ChocoVersionsError(f"no versions found for {pkg!r}")

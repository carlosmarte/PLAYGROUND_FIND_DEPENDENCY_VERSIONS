...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.ChocoVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.ChocoVersionsSDK`` directly.
...
exits:
    python3 cli.py versions git
    python3 cli.py run git --limit 5 --first-only -v
...
class ChocoVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.ChocoVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"source = {self.sdk.effective_index_url() or '(choco default)'}")
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  choco     = {self.cfg.choco_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved choco/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        except sdk.ChocoVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test git        # every version the registry advertises
            test git 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run git --limit 5 --first-only
        """

...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.CpanVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.CpanVersionsSDK`` directly.
...
exits:
    python3 cli.py versions JSON
    python3 cli.py run JSON --limit 5 --first-only -v
...
class CpanVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.CpanVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"index-url = {self.sdk.effective_index_url() or '(cpanm default)'}")
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  cpanm     = {self.cfg.cpanm_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved cpanm/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.CpanVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test JSON         # every version the registry advertises
            test JSON 10      # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run JSON --limit 5 --first-only
        """

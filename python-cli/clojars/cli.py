...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.ClojarsVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.ClojarsVersionsSDK`` directly.
...
exits:
    python3 cli.py versions org.clojure:clojure
    python3 cli.py run org.clojure:clojure --limit 5 --first-only -v
...
class ClojarsVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.ClojarsVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"repo-url = {self.sdk.effective_index_url() or '(clojars default)'}")
    ...
    def do_clojure(self, arg):
        ...
        print(f"clojure-version = {self.cfg.pip_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full mvn output so resolves are debuggable.

        With no argument, toggles. When on, every find/test streams mvn's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  clojure   = {self.cfg.pip_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved mvn/HTTP env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.ClojarsVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test org.clojure:clojure        # every version Clojars advertises
            test org.clojure:clojure 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run org.clojure:clojure --limit 5 --first-only
        """

...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  DockerVersionsSDK  ->  main.py
    external  ->  DockerVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``DockerVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("library/nginx", index_url="registry-1.docker.io", limit=5)
    print(report.installable)          # ['1.27.3', ...]

    versions = sdk.versions("library/nginx")  # just list the tags the registry advertises
    report = sdk.find("library/nginx")        # stop at the first tag that pulls

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("library/nginx")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("library/nginx")  # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.DockerVersionsSDK(index_url="registry-1.docker.io", docker_version="27.3.1")
    s.config.package = "library/nginx"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.DockerVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.DockerVersionsSDK().run(["library/nginx", "--limit", "5", "--first-only"])
"""
...
DEFAULT_DOCKER_VERSION = main.DEFAULT_DOCKER_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use docker's default registry, no
# --registry; limit=None => test every tag).
_UNSET = object()


class DockerVersionsError(RuntimeError):
    """SDK-level failure (e.g. no package set, or no tags to test)."""
...
class Config:
    ...
    An explicit ``None`` means "use docker's own default" (omit ``--registry``
    entirely).
    """
    ...
    docker_version: Optional[str] = DEFAULT_DOCKER_VERSION  # None/"none" => skip the check
    limit: Optional[int] = None
    verbose: bool = False  # stream full docker output so pulls are debuggable
    env: dict = field(default_factory=dict)  # per-call overrides for ENV_DEFAULTS


@dataclass
class Report:
    """Structured outcome of a pull-test run (wraps ``main``'s raw dicts)."""
...
class DockerVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._pip_path = None  # lazily provisioned sandbox docker path

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "DockerVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--registry`` resolves from the
        environment (``_UNSET``), and ``--docker-version none`` skips the
        client-version check.
        """
        ns = main.parse_args(argv)
        docker_version = None if str(ns.docker_version).lower() == "none" else ns.docker_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The registry docker will actually receive (``None`` => docker default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.docker_version``.
        """
        self._pip_path = None
    ...
    def test(self, package=None, limit=_UNSET) -> Report:
        ...
        ``limit=None`` tests every advertised tag.
        """
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise DockerVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise DockerVersionsError(f"no versions found for {pkg!r}")

        docker_path = self.ensure_pip()
        results = main.test_installations(
            docker_path, pkg, index_url, versions, self.config.output,
...
def versions(package, **config) -> List[str]:
    """One-shot: list tags a registry advertises for ``package``."""
    return DockerVersionsSDK(package=package, **config).available_versions()


def versions_output(package, **config) -> dict:
    """One-shot: JSON-able envelope of the versions a registry advertises."""
    return DockerVersionsSDK(package=package, **config).versions_output()


def find(package, **config) -> Report:
    """One-shot: pull-test until the first tag that works."""
    return DockerVersionsSDK(package=package, **config).find()


def test(package, **config) -> Report:
    """One-shot: pull-test tags and write a report."""
    return DockerVersionsSDK(package=package, **config).test()


def run(argv) -> int:
    """One-shot raw passthrough to ``main.main``."""
    return DockerVersionsSDK().run(argv)

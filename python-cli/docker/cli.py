...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.DockerVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.DockerVersionsSDK`` directly.
...
exits:
    python3 cli.py versions library/nginx
    python3 cli.py run library/nginx --limit 5 --first-only -v
...
class DockerVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.DockerVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"registry = {self.sdk.effective_index_url() or '(docker default)'}")
    ...
    def do_docker(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.docker_version = arg
            self.sdk.invalidate_venv()  # re-check on next pull-test
        print(f"docker-version = {self.cfg.docker_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full docker output so pulls are debuggable.

        With no argument, toggles. When on, every find/test streams docker's live
        output (and a copy lands in the report) so you can see why a tag failed
        to pull.
        """
        arg = arg.strip().lower()
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  docker    = {self.cfg.docker_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved docker/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.DockerVersionsError as e:
            print(e)

    def do_test(self, arg):
        """test [PACKAGE] [MAX]  — pull-test tags (newest first), write the JSON report.

        An optional trailing MAX caps how many tags to test this run,
        overriding the session `limit`. Examples:
            test library/nginx        # every tag the registry advertises
            test library/nginx 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run library/nginx --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <package> [--registry HOST] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

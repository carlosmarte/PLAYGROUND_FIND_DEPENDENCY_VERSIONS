...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  GradleVersionsSDK  ->  main.py
    external  ->  GradleVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``GradleVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("com.google.guava:guava", repo_url="https://repo1.maven.org/maven2", limit=5)
    print(report.installable)          # ['33.3.1-jre', ...]

    versions = sdk.versions("com.google.guava:guava")   # just list what the repo advertises
    report = sdk.find("com.google.guava:guava")         # stop at the first version that resolves

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("com.google.guava:guava")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("com.google.guava:guava")       # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.GradleVersionsSDK(index_url="https://repo1.maven.org/maven2", gradle_version="8.10")
    s.config.package = "com.google.guava:guava"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.GradleVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.GradleVersionsSDK().run(["com.google.guava:guava", "--limit", "5", "--first-only"])
"""
...
DEFAULT_GRADLE_VERSION = main.DEFAULT_GRADLE_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use gradle's default, no
# --repo-url; limit=None => test every version).
_UNSET = object()


class GradleVersionsError(RuntimeError):
...
class Config:
    ...
    ``None`` means "use gradle's own default" (mavenCentral(), no extra repo).
    """

    package: Optional[str] = None
    index_url: object = _UNSET
    venv_dir: str = ".gradle-test-home"
    output: str = "installation_report.json"
    pip_version: Optional[str] = DEFAULT_GRADLE_VERSION  # None/"none" => skip the version check
    limit: Optional[int] = None
    verbose: bool = False  # stream full gradle output so resolves are debuggable
...
class GradleVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._pip_path = None  # lazily provisioned Gradle sandbox path

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "GradleVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--repo-url`` resolves from the
        environment (``_UNSET``), and ``--gradle-version none`` skips the
        version check.
        """
        ns = main.parse_args(argv)
        gradle_version = None if str(ns.gradle_version).lower() == "none" else ns.gradle_version
        return cls(Config(
            package=ns.package,
            index_url=_UNSET if ns.repo_url is None else ns.repo_url,
            venv_dir=ns.venv_dir,
            output=ns.output,
            pip_version=gradle_version,
            limit=ns.limit,
            verbose=ns.verbose,
        ))
    ...
    def effective_index_url(self) -> Optional[str]:
        """The repo URL gradle will actually receive (``None`` => mavenCentral())."""
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise GradleVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise GradleVersionsError(f"no versions found for {pkg!r}")

...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.RpmVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.RpmVersionsSDK`` directly.
...
class RpmVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.RpmVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"repository = {self.sdk.effective_index_url() or '(dnf default)'}")
    ...
    def do_dnf(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.dnf_version = arg
            self.sdk.invalidate_venv()  # re-check on next install-test
        print(f"dnf-version = {self.cfg.dnf_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full dnf output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams dnf's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output     = {self.cfg.output}")
        print(f"  venv-dir   = {self.cfg.venv_dir}")
        print(f"  dnf        = {self.cfg.dnf_version}")
        print(f"  verbose    = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved dnf/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.RpmVersionsError as e:
            print(e)
    ...
    def do_run(self, arg):
        ...
        if not argv:
            print("Usage: run <package> [--repository REPO] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.HomebrewVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.HomebrewVersionsSDK`` directly.
...
exits:
    python3 cli.py versions wget
    python3 cli.py run wget --limit 5 --first-only -v
...
class HomebrewVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.HomebrewVersionsSDK()
    ...
    def _resolve_package(self, arg):
        ...
        if not pkg:
            print("No formula set. Use 'package <name>' or pass one inline.")
        elif arg.strip():
            self.cfg.package = pkg  # inline formula becomes the session default
        return pkg

    def _parse_package_and_max(self, arg):
        """Split ``arg`` into ``(formula, max_versions)``.

        A trailing integer token is taken as the MAX cap; the rest is the
        formula (falling back to the session formula when omitted). On an
    ...
    def do_registry(self, arg):
        ...
        print(f"source = {self.sdk.effective_index_url() or '(brew default)'}")
    ...
    def do_brew(self, arg):
        """brew [VERSION|none]  — set or show the brew version the test env checks."""
        arg = arg.strip()
        if arg:
            self.cfg.brew_version = arg
            self.sdk.invalidate_venv()  # re-check on next fetch-test
        print(f"brew-version = {self.cfg.brew_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full brew output so fetches are debuggable.

        With no argument, toggles. When on, every find/test streams brew's live
        output (and a copy lands in the report) so you can see why a token
        failed to fetch.
        """
        arg = arg.strip().lower()
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  brew      = {self.cfg.brew_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved homebrew/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        except sdk.HomebrewVersionsError as e:
            print(e)

    def do_test(self, arg):
        """test [FORMULA] [MAX]  — fetch-test versions (newest first), write the JSON report.

        An optional trailing MAX caps how many versions to test this run,
        overriding the session `limit`. Examples:
            test wget        # every token the registry advertises
            test wget 10     # only the newest 10
            test 10          # newest 10 of the session formula
        """
    ...
    def do_run(self, arg):
        ...
        Example: run wget --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <formula> [--source URL] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

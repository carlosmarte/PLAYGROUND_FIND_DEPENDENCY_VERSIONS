...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  HomebrewVersionsSDK  ->  main.py
    external  ->  HomebrewVersionsSDK  ->  main.py
...
External one-liners::
    ...
    import sdk
    report = sdk.test("wget", source="https://formulae.brew.sh", limit=5)
    print(report.installable)          # ['1.21.4', ...]

    versions = sdk.versions("wget")    # just list what the registry advertises
    report = sdk.find("wget")          # stop at the first token that fetches

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("wget")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("wget")       # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.HomebrewVersionsSDK(source="https://formulae.brew.sh", brew_version="4.3.0")
    s.config.package = "wget"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.HomebrewVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.HomebrewVersionsSDK().run(["wget", "--limit", "5", "--first-only"])
"""
...
DEFAULT_BREW_VERSION = main.DEFAULT_BREW_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use brew's default, no
# --source; limit=None => test every version).
_UNSET = object()


class HomebrewVersionsError(RuntimeError):
    """SDK-level failure (e.g. no formula set, or no versions to test)."""
...
class Config:
    ...
    ``$HOMEBREW_SOURCE > $BREW_REGISTRY_URL > formulae.brew.sh`` chain. An
    explicit ``None`` means "use brew's own default" (omit ``--source`` entirely).
    """
    ...
    brew_version: Optional[str] = DEFAULT_BREW_VERSION  # None/"none" => skip toolchain check
    limit: Optional[int] = None
    verbose: bool = False  # stream full brew output so fetches are debuggable
...
class HomebrewVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        # Accept ``source=`` as a friendly alias for the ``index_url`` field.
        if "source" in overrides:
            overrides["index_url"] = overrides.pop("source")
        self.config = config or Config(**overrides)
        self._venv_dir = None  # lazily provisioned download-cache dir

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "HomebrewVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--source`` resolves from the
        environment (``_UNSET``), and ``--brew-version none`` skips the
        toolchain check.
        """
        ns = main.parse_args(argv)
        brew_version = None if str(ns.brew_version).lower() == "none" else ns.brew_version
        return cls(Config(
            package=ns.formula,
            index_url=_UNSET if ns.source is None else ns.source,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The source URL brew will actually receive (``None`` => its default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.brew_version``.
        """
        self._venv_dir = None
    ...
    def ensure_pip(self) -> str:
        ...
        if self._venv_dir is None:
            bv = self.config.brew_version
            bv = None if (bv is None or str(bv).lower() == "none") else bv
            self._venv_dir = main.setup_venv(
                self.config.venv_dir, bv, self.resolve_env(), verbose=self.config.verbose
            )
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise HomebrewVersionsError("no formula set; pass one or set config.package")
        if package:
            self.config.package = package  # an inline formula becomes the default
        return pkg
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise HomebrewVersionsError(f"no versions found for {pkg!r}")

        venv_dir = self.ensure_pip()
        results = main.test_installations(
            venv_dir, pkg, index_url, versions, self.config.output,

...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.PnpmVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.PnpmVersionsSDK`` directly.
...
exits:
    python3 cli.py versions left-pad
    python3 cli.py run left-pad --limit 5 --first-only -v
...
class PnpmVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.PnpmVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"registry = {self.sdk.effective_index_url() or '(pnpm default)'}")
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  pnpm      = {self.cfg.pnpm_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved pnpm/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.PnpmVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test left-pad        # every version the registry advertises
            test left-pad 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run left-pad --limit 5 --first-only
        """

...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  PnpmVersionsSDK  ->  main.py
    external  ->  PnpmVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``PnpmVersionsSDK`` and override the ``before_probe``
...
Object form (hold one per session, mutate ``.config`` freely)::
    ...
    s = sdk.PnpmVersionsSDK(index_url="https://reg", pnpm_version="9.15.0")
    s.config.package = "left-pad"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.PnpmVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.PnpmVersionsSDK().run(["left-pad", "--limit", "5", "--first-only"])
"""
...
DEFAULT_PNPM_VERSION = main.DEFAULT_PNPM_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use pnpm's default, no
# --registry; limit=None => test every version).
_UNSET = object()


class PnpmVersionsError(RuntimeError):
...
class Config:
    ...
    explicit ``None`` means "use pnpm's own default" (omit ``--registry``
    entirely).
    """

    package: Optional[str] = None
    index_url: object = _UNSET
    venv_dir: str = ".pnpm-test-install"
    output: str = "installation_report.json"
    pnpm_version: Optional[str] = DEFAULT_PNPM_VERSION  # None/"none" => keep pnpm on PATH
    limit: Optional[int] = None
    verbose: bool = False  # stream full pnpm output so installs are debuggable
...
class PnpmVersionsSDK:
    ...
    @classmethod
    def from_argv(cls, argv) -> "PnpmVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--registry`` resolves from the
        environment (``_UNSET``), and ``--pnpm-version none`` keeps the pnpm on
        PATH.
        """
        ns = main.parse_args(argv)
        pnpm_version = None if str(ns.pnpm_version).lower() == "none" else ns.pnpm_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The registry URL pnpm will actually receive (``None`` => pnpm default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.pnpm_version``.
        """
        self._pip_path = None
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise PnpmVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise PnpmVersionsError(f"no versions found for {pkg!r}")

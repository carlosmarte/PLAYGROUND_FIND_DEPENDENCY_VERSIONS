...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  PoetryVersionsSDK  ->  main.py
    external  ->  PoetryVersionsSDK  ->  main.py
...
Object form (hold one per session, mutate ``.config`` freely)::
    ...
    s = sdk.PoetryVersionsSDK(index_url="https://reg/simple", poetry_version="1.8.3")
    s.config.package = "numpy"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.PoetryVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.PoetryVersionsSDK().run(["numpy", "--limit", "5", "--first-only"])
"""
...
DEFAULT_POETRY_VERSION = main.DEFAULT_POETRY_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use poetry's default, no
# --index-url; limit=None => test every version).
_UNSET = object()


class PoetryVersionsError(RuntimeError):
...
class Config:
    ...
    explicit ``None`` means "use poetry's own default" (omit ``--index-url``
    entirely).
    """
    ...
    poetry_version: Optional[str] = DEFAULT_POETRY_VERSION  # None/"none" => keep installed poetry
    limit: Optional[int] = None
    verbose: bool = False  # stream full poetry output so installs are debuggable
...
class PoetryVersionsSDK:
    ...
    @classmethod
    def from_argv(cls, argv) -> "PoetryVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--index-url`` resolves from the
        environment (``_UNSET``), and ``--poetry-version none`` keeps whatever
        poetry is on PATH.
        """
        ns = main.parse_args(argv)
        poetry_version = None if str(ns.poetry_version).lower() == "none" else ns.poetry_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The source URL poetry will actually receive (``None`` => poetry default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.poetry_version``.
        """
        self._venv_dir = None
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise PoetryVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise PoetryVersionsError(f"no versions found for {pkg!r}")

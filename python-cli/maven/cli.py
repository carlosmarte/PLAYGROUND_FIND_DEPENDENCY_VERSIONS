...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.MavenVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.MavenVersionsSDK`` directly.
...
class MavenVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def do_registry(self, arg):
        ...
        print(f"repo-url = {self.sdk.effective_index_url() or '(maven default)'}")
    ...
    def do_maven(self, arg):
        ...
        print(f"maven-version = {self.cfg.pip_version}")
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  maven     = {self.cfg.pip_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved maven/HTTP env vars (os.environ or default)."""

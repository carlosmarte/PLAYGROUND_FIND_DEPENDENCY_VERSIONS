...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  MavenVersionsSDK  ->  main.py
    external  ->  MavenVersionsSDK  ->  main.py
...
results (``Report``), and is built to be:
  ...
  * **extended** — subclass ``MavenVersionsSDK`` and override the ``before_probe``
...
Object form (hold one per session, mutate ``.config`` freely)::
    ...
    s = sdk.MavenVersionsSDK(index_url="https://repo1.maven.org/maven2", maven_version="3.9.9")
    s.config.package = "com.google.guava:guava"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.MavenVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.MavenVersionsSDK().run(["com.google.guava:guava", "--limit", "5", "--first-only"])
"""
...
DEFAULT_MAVEN_VERSION = main.DEFAULT_MAVEN_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use maven's default, no
# --repo-url; limit=None => test every version).
_UNSET = object()


class MavenVersionsError(RuntimeError):
...
class Config:
    ...
    ``$MAVEN_REPO_URL > $JVM_REGISTRY_URL > repo1.maven.org`` chain. An explicit
    ``None`` means "use maven's own default" (omit ``--repo-url`` entirely).
    """
    ...
    pip_version: Optional[str] = DEFAULT_MAVEN_VERSION  # None/"none" => skip the version check
...
class MavenVersionsSDK:
    ...
    def effective_index_url(self) -> Optional[str]:
        """The repo URL mvn will actually receive (``None`` => maven default)."""
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise MavenVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise MavenVersionsError(f"no versions found for {pkg!r}")

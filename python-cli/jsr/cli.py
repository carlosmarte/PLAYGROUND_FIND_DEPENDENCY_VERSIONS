...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.JsrVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.JsrVersionsSDK`` directly.
...
exits:
    python3 cli.py versions @std/encoding
    python3 cli.py run @std/encoding --limit 5 --first-only -v
...
class JsrVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.JsrVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"registry = {self.sdk.effective_index_url() or '(jsr default)'}")
    ...
    def do_jsr(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.jsr_version = arg
            self.sdk.invalidate_venv()  # re-record on next install-test
        print(f"jsr-version = {self.cfg.jsr_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full npx jsr output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams jsr's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  jsr       = {self.cfg.jsr_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved jsr/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.JsrVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test @std/encoding        # every version the registry advertises
            test @std/encoding 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run @std/encoding --limit 5 --first-only
        """

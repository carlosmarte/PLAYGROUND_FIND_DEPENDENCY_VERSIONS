...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  JsrVersionsSDK  ->  main.py
    external  ->  JsrVersionsSDK  ->  main.py
...
lazily provisions the isolated test temp project, returns *structured* results
(``Report``), and is built to be:

  * **extended** — subclass ``JsrVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("@std/encoding", index_url="https://jsr.io", limit=5)
    print(report.installable)          # ['1.0.5', ...]

    versions = sdk.versions("@std/encoding")  # list what the registry advertises
    report = sdk.find("@std/encoding")        # stop at the first version that installs

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("@std/encoding")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("@std/encoding")       # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.JsrVersionsSDK(index_url="https://jsr.io", jsr_version="none")
    s.config.package = "@std/encoding"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.JsrVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.JsrVersionsSDK().run(["@std/encoding", "--limit", "5", "--first-only"])
"""
...
DEFAULT_JSR_VERSION = main.DEFAULT_JSR_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use jsr's default, no
# --registry; limit=None => test every version).
_UNSET = object()


class JsrVersionsError(RuntimeError):
...
class Config:
    ...
    ``$JSR_URL > $JSR_REGISTRY_URL > jsr.io`` chain. An explicit ``None`` means
    "use jsr's own default" (omit ``--registry`` entirely).
    """

    package: Optional[str] = None
    index_url: object = _UNSET
    venv_dir: str = ".jsr-test-install"
    output: str = "installation_report.json"
    jsr_version: Optional[str] = DEFAULT_JSR_VERSION  # None/"none" => keep jsr on PATH
    limit: Optional[int] = None
    verbose: bool = False  # stream full npx jsr output so installs are debuggable
...
class JsrVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._pip_path = None  # lazily provisioned test temp-project dir

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "JsrVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--registry`` resolves from the
        environment (``_UNSET``), and ``--jsr-version none`` keeps the jsr on
        PATH.
        """
        ns = main.parse_args(argv)
        jsr_version = None if str(ns.jsr_version).lower() == "none" else ns.jsr_version
        return cls(Config(
            package=ns.package,
            index_url=_UNSET if ns.index_url is None else ns.index_url,
            venv_dir=ns.venv_dir,
            output=ns.output,
            jsr_version=jsr_version,
            limit=ns.limit,
            verbose=ns.verbose,
        ))
    ...
    def effective_index_url(self) -> Optional[str]:
        """The registry URL jsr will actually receive (``None`` => jsr default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.jsr_version``.
        """
        self._pip_path = None
    ...
    def ensure_pip(self) -> str:
        ...
        if self._pip_path is None:
            jv = self.config.jsr_version
            jv = None if (jv is None or str(jv).lower() == "none") else jv
            self._pip_path = main.setup_venv(
                self.config.venv_dir, jv, self.resolve_env(), verbose=self.config.verbose
            )
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise JsrVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise JsrVersionsError(f"no versions found for {pkg!r}")

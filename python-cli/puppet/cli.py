...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.PuppetVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.PuppetVersionsSDK`` directly.
...
exits:
    python3 cli.py versions puppetlabs-stdlib
    python3 cli.py run puppetlabs-stdlib --limit 5 --first-only -v
...
class PuppetVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.PuppetVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.forge_server = None if arg.lower() == "none" else arg
        print(f"forge-server = {self.sdk.effective_forge_server() or '(puppet default)'}")
    ...
    def do_puppet(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.puppet_version = arg
            self.sdk.invalidate_venv()  # re-check on next install-test
        print(f"puppet-version = {self.cfg.puppet_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full puppet output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams puppet's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output       = {self.cfg.output}")
        print(f"  venv-dir     = {self.cfg.venv_dir}")
        print(f"  puppet       = {self.cfg.puppet_version}")
        print(f"  verbose      = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved puppet/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.PuppetVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test puppetlabs-stdlib      # every version the Forge advertises
            test puppetlabs-stdlib 10   # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run puppetlabs-stdlib --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <package> [--forge-server URL] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

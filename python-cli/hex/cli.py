...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.HexVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.HexVersionsSDK`` directly.
...
exits:
    python3 cli.py versions jason
    python3 cli.py run jason --limit 5 --first-only -v
...
class HexVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.HexVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"index-url = {self.sdk.effective_index_url() or '(hex default)'}")
    ...
    def do_hex(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.hex_version = arg
            self.sdk.invalidate_venv()  # re-check on next fetch-test
        print(f"hex-version = {self.cfg.hex_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full mix output so fetches are debuggable.

        With no argument, toggles. When on, every find/test streams mix's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  hex       = {self.cfg.hex_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved hex/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.HexVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test jason        # every version the registry advertises
            test jason 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run jason --limit 5 --first-only
        """

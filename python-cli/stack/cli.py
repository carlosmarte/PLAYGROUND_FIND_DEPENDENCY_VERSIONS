...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.StackVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.StackVersionsSDK`` directly.
...
class StackVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.StackVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"index-url = {self.sdk.effective_index_url() or '(stack default)'}")
    ...
    def do_stack(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.stack_version = arg
            self.sdk.invalidate_venv()  # re-check on next resolve-test
        print(f"stack-version = {self.cfg.stack_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full stack output so resolves are debuggable.

        With no argument, toggles. When on, every find/test streams stack's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  stack     = {self.cfg.stack_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved stack/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        except sdk.StackVersionsError as e:
            print(e)

...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.NuGetVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.NuGetVersionsSDK`` directly.
...
exits:
    python3 cli.py versions Newtonsoft.Json
    python3 cli.py run Newtonsoft.Json --limit 5 --first-only -v
...
class NuGetVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.NuGetVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"source = {self.sdk.effective_index_url() or '(dotnet default)'}")
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  dotnet    = {self.cfg.dotnet_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved nuget/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.NuGetVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test Newtonsoft.Json        # every version the registry advertises
            test Newtonsoft.Json 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run Newtonsoft.Json --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <package> [--source URL] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

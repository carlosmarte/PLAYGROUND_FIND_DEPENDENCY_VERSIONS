...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  NuGetVersionsSDK  ->  main.py
    external  ->  NuGetVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``NuGetVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("Newtonsoft.Json", source="https://api.nuget.org/v3/index.json", limit=5)
    print(report.installable)          # ['13.0.3', ...]

    versions = sdk.versions("Newtonsoft.Json")   # just list what the registry advertises
    report = sdk.find("Newtonsoft.Json")         # stop at the first version that installs

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("Newtonsoft.Json")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("Newtonsoft.Json")       # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.NuGetVersionsSDK(source="https://api.nuget.org/v3/index.json", dotnet_version="8.0")
    s.config.package = "Newtonsoft.Json"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.NuGetVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.NuGetVersionsSDK().run(["Newtonsoft.Json", "--limit", "5", "--first-only"])
"""
...
DEFAULT_DOTNET_VERSION = main.DEFAULT_DOTNET_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use dotnet's default, no
# --source; limit=None => test every version).
_UNSET = object()


class NuGetVersionsError(RuntimeError):
...
class Config:
    ...
    ``None`` means "use dotnet's own default" (omit ``--source`` entirely).
    """
    ...
    dotnet_version: Optional[str] = DEFAULT_DOTNET_VERSION  # None/"none" => skip toolchain check
    limit: Optional[int] = None
    verbose: bool = False  # stream full dotnet output so installs are debuggable
...
class NuGetVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._venv_dir = None  # lazily provisioned test-project dir

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "NuGetVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--source`` resolves from the
        environment (``_UNSET``), and ``--dotnet-version none`` skips the
        toolchain check.
        """
        ns = main.parse_args(argv)
        dotnet_version = None if str(ns.dotnet_version).lower() == "none" else ns.dotnet_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The source URL dotnet will actually receive (``None`` => its default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.dotnet_version``.
        """
        self._venv_dir = None
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise NuGetVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise NuGetVersionsError(f"no versions found for {pkg!r}")

...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  UvVersionsSDK  ->  main.py
    external  ->  UvVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``UvVersionsSDK`` and override the ``before_probe``
...
Object form (hold one per session, mutate ``.config`` freely)::
    ...
    s = sdk.UvVersionsSDK(index_url="https://reg/simple", uv_version="0.4.18")
    s.config.package = "numpy"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.UvVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.UvVersionsSDK().run(["numpy", "--limit", "5", "--first-only"])
"""
...
DEFAULT_UV_VERSION = main.DEFAULT_UV_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use uv's default, no
# --index-url; limit=None => test every version).
_UNSET = object()


class UvVersionsError(RuntimeError):
...
class Config:
    ...
    ``$UV_INDEX_URL > $PYTHON_REGISTRY_URL > pypi.org`` chain. An explicit
    ``None`` means "use uv's own default" (omit ``--index-url`` entirely).
    """
    ...
    uv_version: Optional[str] = DEFAULT_UV_VERSION  # None/"none" => keep installed uv
    limit: Optional[int] = None
    verbose: bool = False  # stream full uv output so installs are debuggable
...
class UvVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._py_path = None  # lazily provisioned test-venv python

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "UvVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--index-url`` resolves from the
        environment (``_UNSET``), and ``--uv-version none`` keeps whatever uv is
        on PATH.
        """
        ns = main.parse_args(argv)
        uv_version = None if str(ns.uv_version).lower() == "none" else ns.uv_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The index URL uv will actually receive (``None`` => uv default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.uv_version``.
        """
        self._py_path = None

    def ensure_pip(self) -> str:
        """Provision the uv venv once and return its python path."""
        if self._py_path is None:
            uv = self.config.uv_version
            uv = None if (uv is None or str(uv).lower() == "none") else uv
            self._py_path = main.setup_venv(
                self.config.venv_dir, uv, self.resolve_env(), verbose=self.config.verbose
            )
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise UvVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise UvVersionsError(f"no versions found for {pkg!r}")

        py_path = self.ensure_pip()
        results = main.test_installations(
            py_path, pkg, index_url, versions, self.config.output,

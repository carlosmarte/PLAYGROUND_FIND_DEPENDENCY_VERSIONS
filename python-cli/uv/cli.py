...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.UvVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.UvVersionsSDK`` directly.
...
class UvVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.UvVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"index-url = {self.sdk.effective_index_url() or '(uv default)'}")
    ...
    def do_uv(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.uv_version = arg
            self.sdk.invalidate_venv()  # re-check on next install-test
        print(f"uv-version = {self.cfg.uv_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full uv output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams uv's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  uv        = {self.cfg.uv_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved uv/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.UvVersionsError as e:
            print(e)

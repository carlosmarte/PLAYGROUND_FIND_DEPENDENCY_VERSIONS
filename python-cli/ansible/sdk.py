...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  AnsibleVersionsSDK  ->  main.py
    external  ->  AnsibleVersionsSDK  ->  main.py
...
External one-liners::
    ...
    import sdk
    report = sdk.test("community.general", galaxy_server="https://galaxy.example.com", limit=5)
    print(report.installable)          # ['9.0.1', ...]

    versions = sdk.versions("community.general")  # just list what Galaxy advertises
    report = sdk.find("community.general")        # stop at the first that installs

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("community.general")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("community.general")  # {'package', 'galaxy_server', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.AnsibleVersionsSDK(galaxy_server="https://galaxy.example.com", ansible_version="11.1.0")
    s.config.package = "community.general"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.AnsibleVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.AnsibleVersionsSDK().run(["community.general", "--limit", "5", "--first-only"])
"""
...
DEFAULT_ANSIBLE_VERSION = main.DEFAULT_ANSIBLE_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (galaxy_server=None => use ansible-galaxy's default,
# no --server; limit=None => test every version).
_UNSET = object()


class AnsibleVersionsError(RuntimeError):
...
class Config:
    ...
    ``galaxy_server`` carries a third state beyond str/None: the ``_UNSET``
    ...
    chain. An explicit ``None`` means "use ansible-galaxy's own default" (omit
    ``--server`` entirely).
    """

    package: Optional[str] = None
    galaxy_server: object = _UNSET
    venv_dir: str = ".venv-test-install"
    output: str = "installation_report.json"
    ansible_version: Optional[str] = DEFAULT_ANSIBLE_VERSION  # None/"none" => whatever is on PATH
    limit: Optional[int] = None
    verbose: bool = False  # stream full ansible-galaxy output so installs are debuggable
...
class AnsibleVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._install_path = None  # lazily provisioned sandbox collections dir

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "AnsibleVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--galaxy-server`` resolves from the
        environment (``_UNSET``), and ``--ansible-version none`` uses whatever
        ansible-galaxy is on PATH.
        """
        ns = main.parse_args(argv)
        ansible_version = None if str(ns.ansible_version).lower() == "none" else ns.ansible_version
        return cls(Config(
            package=ns.package,
            galaxy_server=_UNSET if ns.galaxy_server is None else ns.galaxy_server,
    ...
    def effective_galaxy_server(self) -> Optional[str]:
        """The Galaxy server ansible-galaxy will actually receive (``None`` => default)."""
        if self.config.galaxy_server is _UNSET:
            return main.resolve_galaxy_server(None, self.resolve_env())
        return self.config.galaxy_server
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.ansible_version``.
        """
    ...
    def ensure_pip(self) -> str:
        ...
        if self._install_path is None:
            av = self.config.ansible_version
            av = None if (av is None or str(av).lower() == "none") else av
            self._install_path = main.setup_venv(
                self.config.venv_dir, av, self.resolve_env(), verbose=self.config.verbose
            )
    ...
    def available_versions(self, package=None, limit=_UNSET) -> List[str]:
        ...
        versions = main.get_available_versions(
            pkg, self.effective_galaxy_server(), self.resolve_env(),
    ...
    def versions_output(self, package=None, limit=_UNSET) -> dict:
        ...
        bare list): wraps it with the package, the effective Galaxy server, and a
        ...
        return {
            ...
            "galaxy_server": self.effective_galaxy_server(),
            "count": len(found),
            "versions": found,
        }
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise AnsibleVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        versions = main.get_available_versions(pkg, galaxy_server, cfg, verbose=self.config.verbose)
        versions = self._apply_limit(versions, limit)
        versions = self.before_probe(pkg, versions)
        if not versions:
            raise AnsibleVersionsError(f"no versions found for {pkg!r}")

        install_path = self.ensure_pip()
        results = main.test_installations(
            install_path, pkg, galaxy_server, versions, self.config.output,
            first_only=first_only, cfg=cfg, verbose=self.config.verbose,
        )
        report = Report(
            package=pkg, galaxy_server=galaxy_server,
...
def versions(package, **config) -> List[str]:
    """One-shot: list versions Galaxy advertises for ``package``."""
    return AnsibleVersionsSDK(package=package, **config).available_versions()


def versions_output(package, **config) -> dict:
    """One-shot: JSON-able envelope of the versions a registry advertises."""
    return AnsibleVersionsSDK(package=package, **config).versions_output()


def find(package, **config) -> Report:
    """One-shot: install-test until the first version that works."""
    return AnsibleVersionsSDK(package=package, **config).find()


def test(package, **config) -> Report:
    """One-shot: install-test versions and write a report."""
    return AnsibleVersionsSDK(package=package, **config).test()


def run(argv) -> int:
    """One-shot raw passthrough to ``main.main``."""
    return AnsibleVersionsSDK().run(argv)

...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.AnsibleVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.AnsibleVersionsSDK`` directly.
...
exits:
    python3 cli.py versions community.general
    python3 cli.py run community.general --limit 5 --first-only -v
...
class AnsibleVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.AnsibleVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.galaxy_server = None if arg.lower() == "none" else arg
        print(f"galaxy-server = {self.sdk.effective_galaxy_server() or '(ansible-galaxy default)'}")
    ...
    def do_ansible(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.ansible_version = arg
            self.sdk.invalidate_venv()  # re-check on next install-test
        print(f"ansible-version = {self.cfg.ansible_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full ansible-galaxy output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams
        ansible-galaxy's live output (and a copy lands in the report) so you can
        see why a version failed to install.
        """
        arg = arg.strip().lower()
    ...
    def do_show(self, arg):
        ...
        print(f"  output        = {self.cfg.output}")
        print(f"  venv-dir      = {self.cfg.venv_dir}")
        print(f"  ansible       = {self.cfg.ansible_version}")
        print(f"  verbose       = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved ansible/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        except sdk.AnsibleVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test community.general      # every version Galaxy advertises
            test community.general 10   # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run community.general --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <package> [--galaxy-server URL] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

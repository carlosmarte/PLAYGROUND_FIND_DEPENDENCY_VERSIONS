...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.GoVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.GoVersionsSDK`` directly.
...
exits:
    python3 cli.py versions github.com/stretchr/testify
    python3 cli.py run github.com/stretchr/testify --limit 5 --first-only -v
...
class GoVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.GoVersionsSDK()
    ...
    def _resolve_package(self, arg):
        ...
        if not pkg:
            print("No module set. Use 'package <name>' or pass one inline.")
        elif arg.strip():
    ...
    def do_registry(self, arg):
        ...
        print(f"proxy = {self.sdk.effective_index_url() or '(go default)'}")
    ...
    def do_go(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.go_version = arg
            self.sdk.invalidate_venv()  # re-assert on next fetch-test
        print(f"go-version = {self.cfg.go_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full go output so fetches are debuggable.

        With no argument, toggles. When on, every find/test streams go's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  go        = {self.cfg.go_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved go/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.GoVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test github.com/stretchr/testify        # every version the proxy advertises
            test github.com/stretchr/testify 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run github.com/stretchr/testify --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <module> [--proxy URL] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  VcpkgVersionsSDK  ->  main.py
    external  ->  VcpkgVersionsSDK  ->  main.py
...
(``Report``), and is built to be:
  ...
  * **extended** — subclass ``VcpkgVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("fmt", index_url="https://raw.githubusercontent.com/microsoft/vcpkg/master", limit=5)
    print(report.installable)          # ['11.0.2', ...]

    versions = sdk.versions("fmt")     # just list what the versions DB advertises
    report = sdk.find("fmt")           # stop at the first version that installs
...
Structured output (call from any consuming script — no console scraping)::
    ...
    sdk.versions_output("fmt")       # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.VcpkgVersionsSDK(index_url="https://raw.githubusercontent.com/microsoft/vcpkg/master", vcpkg_version="2024-10-18")
    s.config.package = "fmt"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.VcpkgVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.VcpkgVersionsSDK().run(["fmt", "--limit", "5", "--first-only"])
"""
...
DEFAULT_VCPKG_VERSION = main.DEFAULT_VCPKG_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use vcpkg's default registry, no
# --registry; limit=None => test every version).
_UNSET = object()


class VcpkgVersionsError(RuntimeError):
...
class Config:
    ...
    explicit ``None`` means "use vcpkg's own default" (omit ``--registry``
    entirely).
    """
    ...
    vcpkg_version: Optional[str] = DEFAULT_VCPKG_VERSION  # None/"none" => skip the check
    limit: Optional[int] = None
    verbose: bool = False  # stream full vcpkg output so installs are debuggable
...
class VcpkgVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._pip_path = None  # lazily provisioned install sandbox dir

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "VcpkgVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--registry`` resolves from the
        environment (``_UNSET``), and ``--vcpkg-version none`` skips the
        tool-version check.
        """
        ns = main.parse_args(argv)
        vcpkg_version = None if str(ns.vcpkg_version).lower() == "none" else ns.vcpkg_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The registry vcpkg will actually receive (``None`` => vcpkg default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.vcpkg_version``.
        """
        self._pip_path = None
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise VcpkgVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise VcpkgVersionsError(f"no versions found for {pkg!r}")
...
def versions(package, **config) -> List[str]:
    """One-shot: list versions the versions DB advertises for ``package``."""
    return VcpkgVersionsSDK(package=package, **config).available_versions()


def versions_output(package, **config) -> dict:
    """One-shot: JSON-able envelope of the versions a registry advertises."""
    return VcpkgVersionsSDK(package=package, **config).versions_output()


def find(package, **config) -> Report:
    """One-shot: install-test until the first version that works."""
    return VcpkgVersionsSDK(package=package, **config).find()


def test(package, **config) -> Report:
    """One-shot: install-test versions and write a report."""
    return VcpkgVersionsSDK(package=package, **config).test()


def run(argv) -> int:
    """One-shot raw passthrough to ``main.main``."""
    return VcpkgVersionsSDK().run(argv)

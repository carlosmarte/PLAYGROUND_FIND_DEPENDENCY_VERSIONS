...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.NpmVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.NpmVersionsSDK`` directly.
...
class NpmVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.NpmVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"registry = {self.sdk.effective_index_url() or '(npm default)'}")
    ...
    def do_npm(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.npm_version = arg
            self.sdk.invalidate_venv()  # re-pin on next install-test
        print(f"npm-version = {self.cfg.npm_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full npm output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams npm's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  npm       = {self.cfg.npm_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved npm/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        except sdk.NpmVersionsError as e:
            print(e)

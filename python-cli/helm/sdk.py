...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  HelmVersionsSDK  ->  main.py
    external  ->  HelmVersionsSDK  ->  main.py
...
results (``Report``), and is built to be:
  ...
  * **extended** — subclass ``HelmVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("bitnami/nginx", index_url="https://charts.bitnami.com/bitnami", limit=5)
    print(report.installable)          # ['18.2.5', ...]

    versions = sdk.versions("bitnami/nginx")  # just list what the repo advertises
    report = sdk.find("bitnami/nginx")        # stop at the first version that pulls

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("bitnami/nginx")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("bitnami/nginx")       # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.HelmVersionsSDK(index_url="https://charts.bitnami.com/bitnami", helm_version="3.16.3")
    s.config.package = "bitnami/nginx"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.HelmVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.HelmVersionsSDK().run(["bitnami/nginx", "--limit", "5", "--first-only"])
"""
...
DEFAULT_HELM_VERSION = main.DEFAULT_HELM_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use helm's default repo, no
# --repo-url; limit=None => test every version).
_UNSET = object()


class HelmVersionsError(RuntimeError):
...
class Config:
    ...
    ``None`` means "use helm's own default" (omit ``--repo-url`` entirely).
    """
    ...
    helm_version: Optional[str] = DEFAULT_HELM_VERSION  # None/"none" => skip the check
    limit: Optional[int] = None
    verbose: bool = False  # stream full helm output so pulls are debuggable
...
class HelmVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._pip_path = None  # lazily provisioned chart-pull destination

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "HelmVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--repo-url`` resolves from the
        environment (``_UNSET``), and ``--helm-version none`` skips the
        binary-version check.
        """
        ns = main.parse_args(argv)
        helm_version = None if str(ns.helm_version).lower() == "none" else ns.helm_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The repo URL helm will actually receive (``None`` => helm default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.helm_version``.
        """
        self._pip_path = None
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise HelmVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise HelmVersionsError(f"no versions found for {pkg!r}")

        dest_path = self.ensure_pip()
        results = main.test_installations(
            dest_path, pkg, index_url, versions, self.config.output,
...
def versions(package, **config) -> List[str]:
    """One-shot: list versions a repo advertises for ``package``."""
    return HelmVersionsSDK(package=package, **config).available_versions()


def versions_output(package, **config) -> dict:
    """One-shot: JSON-able envelope of the versions a registry advertises."""
    return HelmVersionsSDK(package=package, **config).versions_output()


def find(package, **config) -> Report:
    """One-shot: pull-test until the first version that works."""
    return HelmVersionsSDK(package=package, **config).find()


def test(package, **config) -> Report:
    """One-shot: pull-test versions and write a report."""
    return HelmVersionsSDK(package=package, **config).test()


def run(argv) -> int:
    """One-shot raw passthrough to ``main.main``."""
    return HelmVersionsSDK().run(argv)

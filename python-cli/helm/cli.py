...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.HelmVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.HelmVersionsSDK`` directly.
...
exits:
    python3 cli.py versions bitnami/nginx
    python3 cli.py run bitnami/nginx --limit 5 --first-only -v
...
class HelmVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.HelmVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"repo-url = {self.sdk.effective_index_url() or '(helm default)'}")
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  helm      = {self.cfg.helm_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved helm/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.HelmVersionsError as e:
            print(e)

    def do_test(self, arg):
        """test [PACKAGE] [MAX]  — pull-test versions (newest first), write the JSON report.

        An optional trailing MAX caps how many versions to test this run,
        overriding the session `limit`. Examples:
            test bitnami/nginx        # every version the repo advertises
            test bitnami/nginx 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run bitnami/nginx --limit 5 --first-only
        """

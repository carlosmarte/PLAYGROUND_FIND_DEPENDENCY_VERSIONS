...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  AptVersionsSDK  ->  main.py
    external  ->  AptVersionsSDK  ->  main.py
...
lazily provisions the isolated test apt cache, returns *structured* results
(``Report``), and is built to be:

  * **extended** — subclass ``AptVersionsSDK`` and override the ``before_probe``
...
External one-liners::
    ...
    import sdk
    report = sdk.test("bash", index_url="http://deb.debian.org/debian", limit=5)
    print(report.installable)          # ['5.2.15-2+b2', ...]
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.AptVersionsSDK().run(["bash", "--limit", "5", "--first-only"])
"""
...
DEFAULT_APT_VERSION = main.DEFAULT_APT_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use apt's default, no
# --repository; limit=None => test every version).
_UNSET = object()


class AptVersionsError(RuntimeError):
...
class Config:
    ...
    ``$APT_REPOSITORY > $DEBIAN_REGISTRY_URL > Debian mirror`` chain. An explicit
    ``None`` means "use apt's own default" (omit ``--repository`` entirely).
    """

    package: Optional[str] = None
    index_url: object = _UNSET
    venv_dir: str = ".apt-test-install"
    output: str = "installation_report.json"
    apt_version: Optional[str] = DEFAULT_APT_VERSION  # None/"none" => skip the version check
    limit: Optional[int] = None
    verbose: bool = False  # stream full apt output so installs are debuggable
...
class AptVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._cache_path = None  # lazily provisioned test apt cache

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "AptVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--repository`` resolves from the
        environment (``_UNSET``), and ``--apt-version none`` skips the host apt
        version check.
        """
        ns = main.parse_args(argv)
        apt_version = None if str(ns.apt_version).lower() == "none" else ns.apt_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The repo URL apt will actually receive (``None`` => apt default)."""
    ...
    def invalidate_venv(self) -> None:
        """Drop the cached apt cache dir so the next op re-provisions it.

        Call after changing ``config.venv_dir`` or ``config.apt_version``.
        """
        self._cache_path = None

    def ensure_pip(self) -> str:
        """Provision the test apt cache once and return its path."""
        if self._cache_path is None:
            av = self.config.apt_version
            av = None if (av is None or str(av).lower() == "none") else av
            self._cache_path = main.setup_venv(
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise AptVersionsError("no package set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise AptVersionsError(f"no versions found for {pkg!r}")

        cache_path = self.ensure_pip()
        results = main.test_installations(
            cache_path, pkg, index_url, versions, self.config.output,

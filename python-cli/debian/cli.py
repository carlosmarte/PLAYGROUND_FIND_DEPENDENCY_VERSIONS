...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.AptVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.AptVersionsSDK`` directly.
...
class AptVersionsREPL(cmd.Cmd):
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.AptVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"repository = {self.sdk.effective_index_url() or '(apt default)'}")
    ...
    def do_apt(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.apt_version = arg
            self.sdk.invalidate_venv()  # re-check on next install-test
        print(f"apt-version = {self.cfg.apt_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full apt output so installs are debuggable.

        With no argument, toggles. When on, every find/test streams apt's live
    ...
    def do_show(self, arg):
        ...
        print(f"  output     = {self.cfg.output}")
        print(f"  venv-dir   = {self.cfg.venv_dir}")
        print(f"  apt        = {self.cfg.apt_version}")
        print(f"  verbose    = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved apt/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.AptVersionsError as e:
            print(e)

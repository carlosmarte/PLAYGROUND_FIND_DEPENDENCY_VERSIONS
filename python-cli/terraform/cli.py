...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.TerraformVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.TerraformVersionsSDK`` directly.
...
exits:
    python3 cli.py versions hashicorp/aws
    python3 cli.py run hashicorp/aws --limit 5 --first-only -v
...
class TerraformVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.TerraformVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"registry = {self.sdk.effective_index_url() or '(terraform default)'}")
    ...
    def do_terraform(self, arg):
        ...
        arg = arg.strip()
        if arg:
            self.cfg.terraform_version = arg
            self.sdk.invalidate_venv()  # re-check on next init-test
        print(f"terraform-version = {self.cfg.terraform_version}")

    def do_verbose(self, arg):
        """verbose [on|off]  — stream full terraform output so inits are debuggable.

        With no argument, toggles. When on, every find/test streams terraform's
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  venv-dir  = {self.cfg.venv_dir}")
        print(f"  terraform = {self.cfg.terraform_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved terraform/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.TerraformVersionsError as e:
            print(e)

    def do_test(self, arg):
        """test [PACKAGE] [MAX]  — init-test versions (newest first), write the JSON report.

        An optional trailing MAX caps how many versions to test this run,
        overriding the session `limit`. Examples:
            test hashicorp/aws        # every version the registry advertises
            test hashicorp/aws 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run hashicorp/aws --limit 5 --first-only
        """

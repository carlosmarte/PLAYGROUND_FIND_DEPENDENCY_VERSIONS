...
"""Interactive REPL front-end — a thin shell over the SDK.
    ...
    cli.py (this REPL)  ->  sdk.CranVersionsSDK  ->  main.py
...
external caller can do by driving the same ``sdk.CranVersionsSDK`` directly.
...
exits:
    python3 cli.py versions jsonlite
    python3 cli.py run jsonlite --limit 5 --first-only -v
...
class CranVersionsREPL(cmd.Cmd):
    intro = (
    ...
    def __init__(self, client=None):
        ...
        self.sdk = client or sdk.CranVersionsSDK()
    ...
    def do_registry(self, arg):
        ...
        print(f"repos = {self.sdk.effective_index_url() or '(R default)'}")
    ...
    def do_show(self, arg):
        ...
        print(f"  output    = {self.cfg.output}")
        print(f"  lib-dir   = {self.cfg.venv_dir}")
        print(f"  r         = {self.cfg.r_version}")
        print(f"  verbose   = {'on' if self.cfg.verbose else 'off'}")

    def do_env(self, arg):
        """env  — show the resolved R/TLS env vars (os.environ or default)."""
    ...
    def do_find(self, arg):
        ...
        try:
            self.sdk.find(pkg)
        except sdk.CranVersionsError as e:
            print(e)
    ...
    def do_test(self, arg):
        ...
        overriding the session `limit`. Examples:
            test jsonlite        # every version the registry advertises
            test jsonlite 10     # only the newest 10
    ...
    def do_run(self, arg):
        ...
        Example: run jsonlite --limit 5 --first-only
        """
        argv = shlex.split(arg)
        if not argv:
            print("Usage: run <package> [--repos URL] [--limit N] [--first-only]")
            return
        self.sdk.run(argv)

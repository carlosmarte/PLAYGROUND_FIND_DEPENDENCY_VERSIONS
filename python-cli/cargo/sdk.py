...
"""Programmatic SDK over ``main.py`` — the single funnel for every caller.
    ...
    cli.py    ->  CargoVersionsSDK  ->  main.py
    external  ->  CargoVersionsSDK  ->  main.py
...
results (``Report``), and is built to be:
  ...
  * **extended** — subclass ``CargoVersionsSDK`` and override the
...
External one-liners::
    ...
    import sdk
    report = sdk.test("serde", index_url="https://reg", limit=5)
    print(report.installable)          # ['1.0.210', ...]

    versions = sdk.versions("serde")   # just list what the registry advertises
    report = sdk.find("serde")         # stop at the first version that fetches

Structured output (call from any consuming script — no console scraping)::

    report = sdk.test("serde")
    report.to_dict()                   # JSON-able dict (summary + per-version)
    report.to_json()                   # -> str
    report.write_json("report.json")   # -> writes the file, returns the path

    sdk.versions_output("serde")       # {'package', 'index_url', 'count', 'versions'}

Object form (hold one per session, mutate ``.config`` freely)::

    s = sdk.CargoVersionsSDK(index_url="https://reg", cargo_version="1.83.0")
    s.config.package = "serde"
    s.test(limit=10)

Extension example::

    class QuietSDK(sdk.CargoVersionsSDK):
...
Raw passthrough (external -> SDK -> main, argv untouched)::
    ...
    sdk.CargoVersionsSDK().run(["serde", "--limit", "5", "--first-only"])
"""
...
DEFAULT_CARGO_VERSION = main.DEFAULT_CARGO_VERSION
ENV_DEFAULTS = main.ENV_DEFAULTS

# Sentinel for "argument not supplied" — distinct from an explicit ``None``,
# which is itself meaningful (index_url=None => use cargo's default registry, no
# --registry; limit=None => test every version).
_UNSET = object()


class CargoVersionsError(RuntimeError):
    """SDK-level failure (e.g. no crate set, or no versions to test)."""
...
class Config:
    ...
    ``$CARGO_REGISTRIES_CRATES_IO_INDEX > $RUST_REGISTRY_URL > crates.io`` chain.
    An explicit ``None`` means "use cargo's own default" (omit ``--registry``
    entirely).
    """
    ...
    cargo_version: Optional[str] = DEFAULT_CARGO_VERSION  # None/"none" => keep active toolchain
    limit: Optional[int] = None
    verbose: bool = False  # stream full cargo output so fetches are debuggable
...
class CargoVersionsSDK:
    ...
    def __init__(self, config: Optional[Config] = None, **overrides):
        ...
        self._pip_path = None  # lazily provisioned throwaway crate dir

    # -- construction from CLI args ---------------------------------------

    @classmethod
    def from_argv(cls, argv) -> "CargoVersionsSDK":
        """Build an SDK from a ``main``-style argv list (e.g. ``sys.argv[1:]``).

        Mirrors the CLI exactly: an absent ``--registry`` resolves from the
        environment (``_UNSET``), and ``--cargo-version none`` keeps the active
        rustup toolchain.
        """
        ns = main.parse_args(argv)
        cargo_version = None if str(ns.cargo_version).lower() == "none" else ns.cargo_version
        return cls(Config(
            package=ns.package,
    ...
    def effective_index_url(self) -> Optional[str]:
        """The registry URL cargo will actually receive (``None`` => cargo default)."""
    ...
    def invalidate_venv(self) -> None:
        ...
        Call after changing ``config.venv_dir`` or ``config.cargo_version``.
        """
        self._pip_path = None
    ...
    def _require_package(self, package) -> str:
        ...
        if not pkg:
            raise CargoVersionsError("no crate set; pass one or set config.package")
        if package:
    ...
    def _probe(self, package, limit, first_only) -> Report:
        ...
        if not versions:
            raise CargoVersionsError(f"no versions found for {pkg!r}")

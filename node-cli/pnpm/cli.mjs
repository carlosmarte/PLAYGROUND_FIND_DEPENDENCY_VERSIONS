 ...
 *
 *     cli.mjs (this REPL)  ->  PnpmVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `PnpmVersionsSDK` directly.
 *
 ...
 *     node cli.mjs run left-pad --limit 5 --first-only -v
 *
 * Then, at the (pnpm-versions) prompt:
 *     registry https://my-registry.example.com
 *     versions left-pad     # list what the registry advertises
 *     find left-pad         # install-test until the first version that works
 *     test left-pad         # install-test every version, write a JSON report
 *     test left-pad 10      # install-test only the newest 10 versions
 *     verbose on            # stream full pnpm output to debug install failures
...
import { PnpmVersionsSDK, PnpmVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "pnpm-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(pnpm-versions) ";

export class PnpmVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new PnpmVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`registry = ${this.sdk.effectiveIndexUrl() || "(pnpm default)"}`);
  }
  ...
  // pnpm [VERSION|none]  — set or show the pnpm version pinned in the test project.
  do_pnpm(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.pnpmVersion = arg;
      this.sdk.invalidateVenv(); // re-pin on next install-test
    }
    console.log(`pnpm-version = ${this.cfg.pnpmVersion}`);
  }

  // verbose [on|off]  — stream full pnpm output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  pnpm      = ${this.cfg.pnpmVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved pnpm/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof PnpmVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["verbose [on|off]", "stream full pnpm output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved pnpm/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions left-pad       // -> REPL: `versions left-pad`
 *     docker run <image> run left-pad --limit 5  // -> batch via main.main
 *     docker run -it <image>                     // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new PnpmVersionsREPL();
  if (argv.length) {

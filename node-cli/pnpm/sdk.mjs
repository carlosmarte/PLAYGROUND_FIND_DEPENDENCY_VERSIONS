 ...
 *
 *     cli.mjs   ->  PnpmVersionsSDK  ->  main.mjs
 *     external  ->  PnpmVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `PnpmVersionsSDK` and override the `beforeProbe`
 ...
 *
 *     const versions = sdk.versions("left-pad"); // just list what the registry advertises
 ...
 *
 *     const s = new sdk.PnpmVersionsSDK({ indexUrl: "https://reg", pnpmVersion: "9.15.0" });
 *     s.config.package = "left-pad";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.PnpmVersionsSDK {
 ...
 *
 *     await new sdk.PnpmVersionsSDK().run(["left-pad", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_PNPM_VERSION = main.DEFAULT_PNPM_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use pnpm's default, no
// --registry; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class PnpmVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "PnpmVersionsError";
  }
}
 ...
 * explicit `null` means "use pnpm's own default" (omit `--registry` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    pnpmVersion = DEFAULT_PNPM_VERSION, // null/"none" => keep pnpm on PATH
    limit = null,
    verbose = false, // stream full pnpm output so installs are debuggable
...
export class PnpmVersionsSDK {
   ...
   * environment (`UNSET`), and `--pnpm-version none` keeps the pnpm on PATH.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const pnpmVersion = String(ns.pnpmVersion).toLowerCase() === "none" ? null : ns.pnpmVersion;
    return new PnpmVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The registry URL pnpm will actually receive (`null` => pnpm default). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.pnpmVersion`.
   */
  invalidateVenv() {
    this._pipPath = null;
  }
  ...
  ensurePip() {
    if (this._pipPath === null) {
      let pv = this.config.pnpmVersion;
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new PnpmVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new PnpmVersionsError(`no versions found for '${name}'`);
    }

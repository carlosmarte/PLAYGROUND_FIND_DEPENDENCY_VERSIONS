 ...
 *
 *     cli.mjs (this REPL)  ->  AnsibleVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `AnsibleVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions community.general
 *     node cli.mjs run community.general --limit 5 --first-only -v
 *
 * Then, at the (ansible-versions) prompt:
 *     registry https://my-galaxy.example.com   # set the Galaxy server
 *     versions community.general    # list what Galaxy advertises
 *     find community.general        # install-test until the first version that works
 *     test community.general        # install-test every version, write a JSON report
 *     test community.general 10     # install-test only the newest 10 versions
 *     verbose on                    # stream full ansible-galaxy output to debug failures
...
import { AnsibleVersionsSDK, AnsibleVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "ansible-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(ansible-versions) ";

export class AnsibleVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new AnsibleVersionsSDK();
  ...
  // registry [URL]  — set or show the Galaxy server ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.galaxyServer = arg.toLowerCase() === "none" ? null : arg;
    console.log(`galaxy-server = ${this.sdk.effectiveGalaxyServer() || "(ansible-galaxy default)"}`);
  }

  // package [NAME]  — set or show the default collection (namespace.name).
  do_package(arg) {
    arg = arg.trim();
  ...
  // venv [DIR]  — set or show the sandbox collections dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // ansible [VERSION|none]  — set or show the ansible version expected in the test env.
  do_ansible(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.ansibleVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`ansible-version = ${this.cfg.ansibleVersion}`);
  }

  // verbose [on|off]  — stream full ansible-galaxy output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output        = ${this.cfg.output}`);
    console.log(`  venv-dir      = ${this.cfg.venvDir}`);
    console.log(`  ansible       = ${this.cfg.ansibleVersion}`);
    console.log(`  verbose       = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved ansible/TLS env vars (process.env or default).
  do_env() {
  ...
  // versions [PACKAGE]  — list versions Galaxy advertises.
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof AnsibleVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--galaxy-server URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the sandbox collections dir"],
      ["ansible [VERSION|none]", "set or show the ansible version expected in the test env"],
      ["verbose [on|off]", "stream full ansible-galaxy output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved ansible/TLS env vars"],
      ["versions [PACKAGE]", "list versions Galaxy advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions community.general   // -> REPL: `versions ...`
 *     docker run <image> run community.general --limit 5  // -> batch via main.main
 *     docker run -it <image>                          // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new AnsibleVersionsREPL();
  if (argv.length) {

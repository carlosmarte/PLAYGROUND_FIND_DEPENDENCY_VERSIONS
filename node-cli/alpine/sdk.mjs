 ...
 *
 *     cli.mjs   ->  ApkVersionsSDK  ->  main.mjs
 *     external  ->  ApkVersionsSDK  ->  main.mjs
 *
 ...
 * lazily provisions the isolated test apk root, returns *structured* results
 * (`Report`), and is built to be:
 *
 *   * **extended** — subclass `ApkVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("busybox", { indexUrl: "https://dl-cdn.alpinelinux.org/alpine/latest-stable/main", limit: 5 });
 *     console.log(report.installable);        // ['1.36.1-r5', ...]
 *
 *     const versions = sdk.versions("busybox"); // just list what the repository advertises
 *     const report = await sdk.find("busybox"); // stop at the first version that installs
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("busybox");
 ...
 *
 *     sdk.versionsOutput("busybox");      // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.ApkVersionsSDK({ indexUrl: "https://reg/main", apkVersion: "2.14.4" });
 *     s.config.package = "busybox";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.ApkVersionsSDK {
 ...
 *
 *     await new sdk.ApkVersionsSDK().run(["busybox", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_APK_VERSION = main.DEFAULT_APK_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use apk's default, no
// --repository; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class ApkVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "ApkVersionsError";
  }
}
 ...
 * `$APK_REPOSITORY > $ALPINE_REGISTRY_URL > Alpine mirror` chain. An explicit
 * `null` means "use apk's own default" (omit `--repository` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    apkVersion = DEFAULT_APK_VERSION, // null/"none" => skip the version check
    limit = null,
    verbose = false, // stream full apk output so installs are debuggable
...
export class ApkVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._rootPath = null; // lazily provisioned test apk root
  }
   ...
   * environment (`UNSET`), and `--apk-version none` skips the host apk
   * version check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const apkVersion = String(ns.apkVersion).toLowerCase() === "none" ? null : ns.apkVersion;
    return new ApkVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The repo URL apk will actually receive (`null` => apk default). */
  effectiveIndexUrl() {
  ...
  /**
   * Drop the cached apk root so the next op re-provisions it.
   *
   * Call after changing `config.venvDir` or `config.apkVersion`.
   */
  invalidateVenv() {
    this._rootPath = null;
  }

  /** Provision the test apk root once and return its path. */
  ensurePip() {
    if (this._rootPath === null) {
      let av = this.config.apkVersion;
      av = av === null || String(av).toLowerCase() === "none" ? null : av;
      this._rootPath = main.setupVenv(this.config.venvDir, av, this.resolveEnv(), this.config.verbose);
    }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new ApkVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new ApkVersionsError(`no versions found for '${name}'`);
    }

    const rootPath = this.ensurePip();
    const results = await main.testInstallations(
      rootPath, name, indexUrl, versions, this.config.output,

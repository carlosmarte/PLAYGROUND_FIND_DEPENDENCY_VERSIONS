 ...
 *
 *     cli.mjs (this REPL)  ->  ApkVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `ApkVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions busybox
 *     node cli.mjs run busybox --limit 5 --first-only -v
 *
 * Then, at the (apk-versions) prompt:
 *     registry https://dl-cdn.alpinelinux.org/alpine/latest-stable/main
 *     versions busybox      # list what the repository advertises
 *     find busybox          # install-test until the first version that works
 *     test busybox          # install-test every version, write a JSON report
 *     test busybox 10       # install-test only the newest 10 versions
 *     verbose on            # stream full apk output to debug install failures
...
import { ApkVersionsSDK, ApkVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "apk-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(apk-versions) ";

export class ApkVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new ApkVersionsSDK();
  ...
  // registry [URL]  — set or show the apk repository URL ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`repository = ${this.sdk.effectiveIndexUrl() || "(apk default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test apk root dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // apk [VERSION|none]  — set or show the apk-tools version expected in the test env.
  do_apk(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.apkVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`apk-version = ${this.cfg.apkVersion}`);
  }

  // verbose [on|off]  — stream full apk output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output     = ${this.cfg.output}`);
    console.log(`  venv-dir   = ${this.cfg.venvDir}`);
    console.log(`  apk        = ${this.cfg.apkVersion}`);
    console.log(`  verbose    = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved apk/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof ApkVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test apk root dir"],
      ["apk [VERSION|none]", "set or show the apk-tools version expected in the test env"],
      ["verbose [on|off]", "stream full apk output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved apk/TLS env vars"],
      ["versions [PACKAGE]", "list versions the repository advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions busybox       // -> REPL: `versions busybox`
 *     docker run <image> run busybox --limit 5  // -> batch via main.main
 *     docker run -it <image>                     // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new ApkVersionsREPL();
  if (argv.length) {

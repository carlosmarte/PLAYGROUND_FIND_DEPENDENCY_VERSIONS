 ...
 *
 *     cli.mjs (this REPL)  ->  VagrantVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `VagrantVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions hashicorp/bionic64
 *     node cli.mjs run hashicorp/bionic64 --limit 5 --first-only -v
 *
 * Then, at the (vagrant-versions) prompt:
 *     registry https://my-vagrant.example.com   # set the Vagrant Cloud server
 *     versions hashicorp/bionic64    # list what the Cloud advertises
 *     find hashicorp/bionic64        # install-test until the first version that works
 *     test hashicorp/bionic64        # install-test every version, write a JSON report
 *     test hashicorp/bionic64 10     # install-test only the newest 10 versions
 *     verbose on                     # stream full vagrant output to debug failures
...
import { VagrantVersionsSDK, VagrantVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "vagrant-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(vagrant-versions) ";

export class VagrantVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new VagrantVersionsSDK();
  ...
  _resolvePackage(arg) {
    ...
    if (!pkg) {
      console.log("No package set. Use 'package <name>' or pass one inline.");
    } else if (arg.trim()) {
      this.cfg.package = pkg; // inline package becomes the session default
    }
    return pkg;
  }

  // Split `arg` into `[package, maxVersions]`. A trailing integer token is the
  // MAX cap; the rest is the package. On invalid MAX, prints and returns nulls.
  ...
  // registry [URL]  — set or show the Vagrant Cloud server ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.vagrantServer = arg.toLowerCase() === "none" ? null : arg;
    console.log(`vagrant-server = ${this.sdk.effectiveVagrantServer() || "(vagrant default)"}`);
  }

  // package [NAME]  — set or show the default box (user/box).
  do_package(arg) {
    arg = arg.trim();
  ...
  // venv [DIR]  — set or show the sandbox VAGRANT_HOME dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // vagrant [VERSION|none]  — set or show the vagrant version expected in the test env.
  do_vagrant(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.vagrantVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`vagrant-version = ${this.cfg.vagrantVersion}`);
  }

  // verbose [on|off]  — stream full vagrant output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output         = ${this.cfg.output}`);
    console.log(`  venv-dir       = ${this.cfg.venvDir}`);
    console.log(`  vagrant        = ${this.cfg.vagrantVersion}`);
    console.log(`  verbose        = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved vagrant/TLS env vars (process.env or default).
  do_env() {
  ...
  // versions [PACKAGE]  — list versions the Vagrant Cloud advertises.
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof VagrantVersionsError) console.log(e.message);
      else throw e;
    }
  }

  // test [PACKAGE] [MAX]  — install-test versions (newest first), write report.
  async do_test(arg) {
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--vagrant-server URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the sandbox VAGRANT_HOME dir"],
      ["vagrant [VERSION|none]", "set or show the vagrant version expected in the test env"],
      ["verbose [on|off]", "stream full vagrant output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved vagrant/TLS env vars"],
      ["versions [PACKAGE]", "list versions the Vagrant Cloud advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
      ["test [PACKAGE] [MAX]", "install-test versions, write the JSON report"],
 ...
 *
 *     docker run <image> versions hashicorp/bionic64   // -> REPL: `versions ...`
 *     docker run <image> run hashicorp/bionic64 --limit 5  // -> batch via main.main
 *     docker run -it <image>                           // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new VagrantVersionsREPL();
  if (argv.length) {

 ...
 *
 *     cli.mjs (this REPL)  ->  JsrVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `JsrVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions @std/encoding
 *     node cli.mjs run @std/encoding --limit 5 --first-only -v
 *
 * Then, at the (jsr-versions) prompt:
 *     registry https://jsr.io
 *     versions @std/encoding   # list what the registry advertises
 *     find @std/encoding       # install-test until the first version that works
 *     test @std/encoding       # install-test every version, write a JSON report
 *     test @std/encoding 10    # install-test only the newest 10 versions
 *     verbose on               # stream full npx jsr output to debug install failures
...
import { JsrVersionsSDK, JsrVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "jsr-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(jsr-versions) ";

export class JsrVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new JsrVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`registry = ${this.sdk.effectiveIndexUrl() || "(jsr default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test temp-project dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // jsr [VERSION|none]  — set or show the jsr/deno version recorded for the test project.
  do_jsr(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.jsrVersion = arg;
      this.sdk.invalidateVenv(); // re-record on next install-test
    }
    console.log(`jsr-version = ${this.cfg.jsrVersion}`);
  }

  // verbose [on|off]  — stream full npx jsr output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  jsr       = ${this.cfg.jsrVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved jsr/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof JsrVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--registry URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test temp-project dir"],
      ["jsr [VERSION|none]", "set or show the jsr/deno version recorded for the test project"],
      ["verbose [on|off]", "stream full npx jsr output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved jsr/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions @std/encoding  // -> REPL: `versions @std/encoding`
 *     docker run <image> run @std/encoding --limit 5  // -> batch via main.main
 *     docker run -it <image>                      // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new JsrVersionsREPL();
  if (argv.length) {

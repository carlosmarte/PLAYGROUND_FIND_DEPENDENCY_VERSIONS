 ...
 *
 *     cli.mjs   ->  DockerVersionsSDK  ->  main.mjs
 *     external  ->  DockerVersionsSDK  ->  main.mjs
 *
 ...
 * lazily provisions the isolated pull sandbox, returns *structured* results
 * (`Report`), and is built to be:
 *
 *   * **extended** — subclass `DockerVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("library/nginx", { indexUrl: "registry-1.docker.io", limit: 5 });
 *     console.log(report.installable);        // ['1.27.3', ...]
 *
 *     const versions = await sdk.versions("library/nginx"); // just list the tags the registry advertises
 *     const report = await sdk.find("library/nginx");       // stop at the first tag that pulls
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("library/nginx");
 ...
 *
 *     sdk.versionsOutput("library/nginx");        // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.DockerVersionsSDK({ indexUrl: "registry-1.docker.io", dockerVersion: "27.3.1" });
 *     s.config.package = "library/nginx";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.DockerVersionsSDK {
 ...
 *
 *     await new sdk.DockerVersionsSDK().run(["library/nginx", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_DOCKER_VERSION = main.DEFAULT_DOCKER_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use docker's default registry, no
// --registry; limit=null => test every tag).
export const UNSET = Symbol("UNSET");

export class DockerVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "DockerVersionsError";
  }
}
 ...
 * `$DOCKER_REGISTRY > $CONTAINER_REGISTRY_URL > registry-1.docker.io` chain.
 * An explicit `null` means "use docker's own default" (omit `--registry`
 * entirely).
 */
export class Config {
  constructor({
    ...
    dockerVersion = DEFAULT_DOCKER_VERSION, // null/"none" => skip the check
    limit = null,
    verbose = false, // stream full docker output so pulls are debuggable
...
/** Structured outcome of a pull-test run (wraps `main`'s raw objects). */
export class Report {
  ...
  /** Tags that pulled cleanly, newest-first. */
  get installable() {
    return this.results.filter((r) => r.status === "success").map((r) => r.version);
  }

  /** Tags that failed to pull. */
  get failed() {
    return this.results.filter((r) => r.status !== "success").map((r) => r.version);
  }
...
export class DockerVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._pipPath = null; // lazily provisioned sandbox docker path
  }
   ...
   * environment (`UNSET`), and `--docker-version none` skips the client-version
   * check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const dockerVersion = String(ns.dockerVersion).toLowerCase() === "none" ? null : ns.dockerVersion;
    return new DockerVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The registry docker will actually receive (`null` => docker default). */
  effectiveIndexUrl() {
  ...
  /**
   * Drop the cached sandbox docker path so the next op re-provisions it.
   *
   * Call after changing `config.venvDir` or `config.dockerVersion`.
   */
  invalidateVenv() {
    this._pipPath = null;
  }

  /** Provision the pull sandbox once and return its docker path. */
  ensurePip() {
    if (this._pipPath === null) {
      let dv = this.config.dockerVersion;
      dv = dv === null || String(dv).toLowerCase() === "none" ? null : dv;
      this._pipPath = main.setupVenv(this.config.venvDir, dv, this.resolveEnv(), this.config.verbose);
    }
  ...
  /** List tags the registry advertises (newest-first), capped by limit. */
  ...
  /** Pull-test until the first tag that works; return a `Report`. */
  find(pkg = null) {
    return this._probe(pkg, UNSET, true);
  }

  /**
   * Pull-test tags (newest-first), write the JSON report, return it.
   *
   * An explicit `limit` overrides `config.limit` for this call only;
   * `limit=null` tests every advertised tag.
   */
  ...
  /** Hook: inspect/filter the tag list before testing. Return the list. */
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new DockerVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new DockerVersionsError(`no versions found for '${name}'`);
    }

    const dockerPath = this.ensurePip();
    const results = await main.testInstallations(
      dockerPath, name, indexUrl, versions, this.config.output,
...
/** One-shot: list tags a registry advertises for `pkg`. */
export function versions(pkg, config = {}) {
  return new DockerVersionsSDK({ package: pkg, ...config }).availableVersions();
}

/** One-shot: JSON-able envelope of the versions a registry advertises. */
export function versionsOutput(pkg, config = {}) {
  return new DockerVersionsSDK({ package: pkg, ...config }).versionsOutput();
}

/** One-shot: pull-test until the first tag that works. */
export function find(pkg, config = {}) {
  return new DockerVersionsSDK({ package: pkg, ...config }).find();
}

/** One-shot: pull-test tags and write a report. */
export function test(pkg, config = {}) {
  return new DockerVersionsSDK({ package: pkg, ...config }).test();
}

/** One-shot raw passthrough to `main.main`. */
export function run(argv) {
  return new DockerVersionsSDK().run(argv);
}

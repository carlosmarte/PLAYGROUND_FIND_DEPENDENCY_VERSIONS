 ...
 *
 *     cli.mjs (this REPL)  ->  DockerVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `DockerVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions library/nginx
 *     node cli.mjs run library/nginx --limit 5 --first-only -v
 *
 * Then, at the (docker-versions) prompt:
 *     registry registry-1.docker.io
 *     versions library/nginx   # list the tags the registry advertises
 *     find library/nginx       # pull-test until the first tag that works
 *     test library/nginx       # pull-test every tag, write a JSON report
 *     test library/nginx 10    # pull-test only the newest 10 tags
 *     verbose on               # stream full docker output to debug pull failures
...
import { DockerVersionsSDK, DockerVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "docker-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(docker-versions) ";

export class DockerVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new DockerVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`registry = ${this.sdk.effectiveIndexUrl() || "(docker default)"}`);
  }

  // package [NAME]  — set or show the default repository.
  do_package(arg) {
    arg = arg.trim();
  ...
  // limit [N|none]  — only probe the newest N tags ('none' = all).
  do_limit(arg) {
  ...
  // venv [DIR]  — set or show the pull sandbox dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // docker [VERSION|none]  — set or show the docker client version to expect.
  do_docker(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.dockerVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next pull-test
    }
    console.log(`docker-version = ${this.cfg.dockerVersion}`);
  }

  // verbose [on|off]  — stream full docker output so pulls are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  docker    = ${this.cfg.dockerVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved docker/TLS env vars (process.env or default).
  do_env() {
  ...
  // versions [PACKAGE]  — list tags the registry advertises.
  ...
  // find [PACKAGE]  — pull-test until the first tag that works.
  async do_find(arg) {
    const pkg = this._resolvePackage(arg);
    if (!pkg) return;
    try {
      ...
      if (e instanceof DockerVersionsError) console.log(e.message);
      else throw e;
    }
  }

  // test [PACKAGE] [MAX]  — pull-test tags (newest first), write report.
  async do_test(arg) {
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N tags ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the pull sandbox dir"],
      ["docker [VERSION|none]", "set or show the docker client version to expect"],
      ["verbose [on|off]", "stream full docker output so pulls are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved docker/TLS env vars"],
      ["versions [PACKAGE]", "list tags the registry advertises"],
      ["find [PACKAGE]", "pull-test until the first tag that works"],
      ["test [PACKAGE] [MAX]", "pull-test tags, write the JSON report"],
 ...
 *
 *     docker run <image> versions library/nginx       // -> REPL: `versions library/nginx`
 *     docker run <image> run library/nginx --limit 5  // -> batch via main.main
 *     docker run -it <image>                           // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new DockerVersionsREPL();
  if (argv.length) {

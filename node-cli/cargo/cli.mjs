 ...
 *
 *     cli.mjs (this REPL)  ->  CargoVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `CargoVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions serde
 *     node cli.mjs run serde --limit 5 --first-only -v
 *
...
import { CargoVersionsSDK, CargoVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const PROMPT = "(cargo-versions) ";

export class CargoVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new CargoVersionsSDK();
  ...
  _resolvePackage(arg) {
    ...
    if (!pkg) {
      console.log("No crate set. Use 'package <name>' or pass one inline.");
    } else if (arg.trim()) {
  ...
  do_registry(arg) {
    ...
    console.log(`registry = ${this.sdk.effectiveIndexUrl() || "(cargo default)"}`);
  }
  ...
  do_cargo(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.cargoVersion = arg;
      this.sdk.invalidateVenv(); // re-assert on next fetch-test
    }
    console.log(`cargo-version = ${this.cfg.cargoVersion}`);
  }
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  cargo     = ${this.cfg.cargoVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved cargo/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof CargoVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <crate> [--registry URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the throwaway test crate dir"],
      ["cargo [VERSION|none]", "set or show the cargo version asserted in the test crate"],
      ["verbose [on|off]", "stream full cargo output so fetches are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved cargo/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "fetch-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new CargoVersionsREPL();
  if (argv.length) {

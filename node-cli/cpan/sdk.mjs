 ...
 *
 *     cli.mjs   ->  CpanVersionsSDK  ->  main.mjs
 *     external  ->  CpanVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `CpanVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("JSON", { indexUrl: "https://www.cpan.org", limit: 5 });
 *     console.log(report.installable);        // ['4.10', ...]
 *
 *     const versions = await sdk.versions("JSON"); // just list what the registry advertises
 *     const report = await sdk.find("JSON");       // stop at the first version that installs
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("JSON");
 ...
 *
 *     sdk.versionsOutput("JSON");        // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.CpanVersionsSDK({ indexUrl: "https://www.cpan.org", cpanmVersion: "1.7047" });
 *     s.config.package = "JSON";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.CpanVersionsSDK {
 ...
 *
 *     await new sdk.CpanVersionsSDK().run(["JSON", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_CPANM_VERSION = main.DEFAULT_CPANM_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use cpanm's default, no
// --mirror; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class CpanVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "CpanVersionsError";
  }
}
 ...
 * `null` means "use cpanm's own default" (omit `--mirror` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    cpanmVersion = DEFAULT_CPANM_VERSION, // null/"none" => skip version check
    limit = null,
    verbose = false, // stream full cpanm output so installs are debuggable
    ...
    this.output = output;
    this.cpanmVersion = cpanmVersion;
    this.limit = limit;
...
export class CpanVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._sandbox = null; // lazily provisioned test local-lib dir
  }
   ...
   * environment (`UNSET`), and `--cpanm-version none` skips the sandbox
   * version check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const cpanmVersion = String(ns.cpanmVersion).toLowerCase() === "none" ? null : ns.cpanmVersion;
    return new CpanVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The index URL cpanm will actually receive (`null` => cpanm default). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.cpanmVersion`.
   */
  invalidateVenv() {
    this._sandbox = null;
  }
  ...
  ensurePip() {
    if (this._sandbox === null) {
      let cv = this.config.cpanmVersion;
      cv = cv === null || String(cv).toLowerCase() === "none" ? null : cv;
      this._sandbox = main.setupVenv(this.config.venvDir, cv, this.resolveEnv(), this.config.verbose);
    }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new CpanVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new CpanVersionsError(`no versions found for '${name}'`);
    }

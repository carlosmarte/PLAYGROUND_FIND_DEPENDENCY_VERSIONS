 ...
 *
 *     cli.mjs (this REPL)  ->  CpanVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `CpanVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions JSON
 *     node cli.mjs run JSON --limit 5 --first-only -v
 *
 * Then, at the (cpan-versions) prompt:
 *     registry https://my-registry.example.com
 *     versions JSON         # list what the registry advertises
 *     find JSON             # install-test until the first version that works
 *     test JSON             # install-test every version, write a JSON report
 *     test JSON 10          # install-test only the newest 10 versions
 *     verbose on            # stream full cpanm output to debug install failures
...
import { CpanVersionsSDK, CpanVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "cpan-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(cpan-versions) ";

export class CpanVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new CpanVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`index-url = ${this.sdk.effectiveIndexUrl() || "(cpanm default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test local-lib sandbox dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // cpanm [VERSION|none]  — set or show the cpanm version expected in the sandbox.
  do_cpanm(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.cpanmVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`cpanm-version = ${this.cfg.cpanmVersion}`);
  }

  // verbose [on|off]  — stream full cpanm output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  cpanm     = ${this.cfg.cpanmVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved cpanm/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof CpanVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--index-url URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test local-lib sandbox dir"],
      ["cpanm [VERSION|none]", "set or show the cpanm version expected in the sandbox"],
      ["verbose [on|off]", "stream full cpanm output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved cpanm/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions JSON         // -> REPL: `versions JSON`
 *     docker run <image> run JSON --limit 5    // -> batch via main.main
 *     docker run -it <image>                    // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new CpanVersionsREPL();
  if (argv.length) {

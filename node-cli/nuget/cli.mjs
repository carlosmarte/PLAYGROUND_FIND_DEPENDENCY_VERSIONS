 ...
 *
 *     cli.mjs (this REPL)  ->  NuGetVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `NuGetVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions Newtonsoft.Json
 *     node cli.mjs run Newtonsoft.Json --limit 5 --first-only -v
 *
 * Then, at the (nuget-versions) prompt:
 *     registry https://api.nuget.org/v3/index.json
 *     versions Newtonsoft.Json   # list what the registry advertises
 *     find Newtonsoft.Json       # install-test until the first version that works
 *     test Newtonsoft.Json       # install-test every version, write a JSON report
 *     test Newtonsoft.Json 10    # install-test only the newest 10 versions
 *     verbose on                 # stream full dotnet output to debug install failures
...
import { NuGetVersionsSDK, NuGetVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "nuget-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(nuget-versions) ";

export class NuGetVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new NuGetVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`source = ${this.sdk.effectiveIndexUrl() || "(dotnet default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test project dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // dotnet [VERSION|none]  — set or show the dotnet SDK version the test env checks.
  do_dotnet(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.dotnetVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`dotnet-version = ${this.cfg.dotnetVersion}`);
  }

  // verbose [on|off]  — stream full dotnet output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  dotnet    = ${this.cfg.dotnetVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved nuget/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof NuGetVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--source URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test project dir"],
      ["dotnet [VERSION|none]", "set or show the dotnet SDK version the test env checks"],
      ["verbose [on|off]", "stream full dotnet output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved nuget/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions Newtonsoft.Json   // -> REPL: `versions ...`
 *     docker run <image> run Newtonsoft.Json --limit 5  // -> batch via main.main
 *     docker run -it <image>                        // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new NuGetVersionsREPL();
  if (argv.length) {

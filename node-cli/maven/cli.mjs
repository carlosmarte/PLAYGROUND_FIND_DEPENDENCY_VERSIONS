 ...
 *
 *     cli.mjs (this REPL)  ->  MavenVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `MavenVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions com.google.guava:guava
 *     node cli.mjs run com.google.guava:guava --limit 5 --first-only -v
 *
 * Then, at the (maven-versions) prompt:
...
import { MavenVersionsSDK, MavenVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const PROMPT = "(maven-versions) ";

export class MavenVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new MavenVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`repo-url = ${this.sdk.effectiveIndexUrl() || "(maven default)"}`);
  }
  ...
  // maven [VERSION|none]  — set or show the maven version verified in the test env.
  do_maven(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.mavenVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next resolve-test
    }
    console.log(`maven-version = ${this.cfg.mavenVersion}`);
  }
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  maven     = ${this.cfg.mavenVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved maven/HTTP env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof MavenVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["verbose [on|off]", "stream full mvn output so resolves are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved maven/HTTP env vars"],
      ["versions [PACKAGE]", "list versions the repository advertises"],
      ["find [PACKAGE]", "resolve-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new MavenVersionsREPL();
  if (argv.length) {

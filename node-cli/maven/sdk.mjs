 ...
 *
 *     cli.mjs   ->  MavenVersionsSDK  ->  main.mjs
 *     external  ->  MavenVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `MavenVersionsSDK` and override the `beforeProbe`
 ...
 *
 *     const s = new sdk.MavenVersionsSDK({ indexUrl: "https://repo1.maven.org/maven2", mavenVersion: "3.9.9" });
 *     s.config.package = "com.google.guava:guava";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.MavenVersionsSDK {
 ...
 *
 *     await new sdk.MavenVersionsSDK().run(["com.google.guava:guava", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_MAVEN_VERSION = main.DEFAULT_MAVEN_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use maven's default, no
// --repo-url; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class MavenVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "MavenVersionsError";
  }
}
 ...
 * `null` means "use maven's own default" (omit `--repo-url` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    mavenVersion = DEFAULT_MAVEN_VERSION, // null/"none" => skip the version check
    limit = null,
...
export class MavenVersionsSDK {
   ...
   * environment (`UNSET`), and `--maven-version none` skips the version check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const mavenVersion = String(ns.mavenVersion).toLowerCase() === "none" ? null : ns.mavenVersion;
    return new MavenVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The repo URL mvn will actually receive (`null` => maven default). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.mavenVersion`.
   */
  invalidateVenv() {
    this._pipPath = null;
  }
  ...
  ensurePip() {
    if (this._pipPath === null) {
      let pv = this.config.mavenVersion;
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new MavenVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new MavenVersionsError(`no versions found for '${name}'`);
    }

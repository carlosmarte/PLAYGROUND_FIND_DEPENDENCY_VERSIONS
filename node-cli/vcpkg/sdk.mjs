 ...
 *
 *     cli.mjs   ->  VcpkgVersionsSDK  ->  main.mjs
 *     external  ->  VcpkgVersionsSDK  ->  main.mjs
 *
 ...
 * lazily provisions the isolated install sandbox, returns *structured* results
 * (`Report`), and is built to be:
 *
 *   * **extended** — subclass `VcpkgVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("fmt", { indexUrl: "https://raw.githubusercontent.com/microsoft/vcpkg/master", limit: 5 });
 *     console.log(report.installable);        // ['11.0.2', ...]
 *
 *     const versions = await sdk.versions("fmt"); // just list what the versions DB advertises
 *     const report = await sdk.find("fmt");       // stop at the first version that installs
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("fmt");
 ...
 *
 *     sdk.versionsOutput("fmt");          // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.VcpkgVersionsSDK({ indexUrl: "https://raw.githubusercontent.com/microsoft/vcpkg/master", vcpkgVersion: "2024-10-18" });
 *     s.config.package = "fmt";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.VcpkgVersionsSDK {
 ...
 *
 *     await new sdk.VcpkgVersionsSDK().run(["fmt", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_VCPKG_VERSION = main.DEFAULT_VCPKG_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use vcpkg's default registry, no
// --registry; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class VcpkgVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "VcpkgVersionsError";
  }
}
 ...
 * `$VCPKG_REGISTRY > $PORT_REGISTRY_URL > microsoft/vcpkg master` chain. An
 * explicit `null` means "use vcpkg's own default" (omit `--registry` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    vcpkgVersion = DEFAULT_VCPKG_VERSION, // null/"none" => skip the check
    limit = null,
    verbose = false, // stream full vcpkg output so installs are debuggable
    ...
    this.output = output;
    this.vcpkgVersion = vcpkgVersion;
    this.limit = limit;
...
export class VcpkgVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._pipPath = null; // lazily provisioned install sandbox dir
  }
   ...
   * environment (`UNSET`), and `--vcpkg-version none` skips the tool-version
   * check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const vcpkgVersion = String(ns.vcpkgVersion).toLowerCase() === "none" ? null : ns.vcpkgVersion;
    return new VcpkgVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The registry vcpkg will actually receive (`null` => vcpkg default). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.vcpkgVersion`.
   */
  invalidateVenv() {
    this._pipPath = null;
  }

  /** Provision the install sandbox once and return its path. */
  ensurePip() {
    if (this._pipPath === null) {
      let vv = this.config.vcpkgVersion;
      vv = vv === null || String(vv).toLowerCase() === "none" ? null : vv;
      this._pipPath = main.setupVenv(this.config.venvDir, vv, this.resolveEnv(), this.config.verbose);
    }
  ...
  /** List versions the versions DB advertises (newest-first), capped by limit. */
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new VcpkgVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new VcpkgVersionsError(`no versions found for '${name}'`);
    }

 ...
 *
 *     cli.mjs (this REPL)  ->  VcpkgVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `VcpkgVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions fmt
 *     node cli.mjs run fmt --limit 5 --first-only -v
 *
...
import { VcpkgVersionsSDK, VcpkgVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const PROMPT = "(vcpkg-versions) ";

export class VcpkgVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new VcpkgVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`registry = ${this.sdk.effectiveIndexUrl() || "(vcpkg default)"}`);
  }
  ...
  do_vcpkg(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.vcpkgVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`vcpkg-version = ${this.cfg.vcpkgVersion}`);
  }
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  vcpkg     = ${this.cfg.vcpkgVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved vcpkg/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof VcpkgVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the install sandbox dir"],
      ["vcpkg [VERSION|none]", "set or show the vcpkg version to expect"],
      ["verbose [on|off]", "stream full vcpkg output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved vcpkg/TLS env vars"],
      ["versions [PACKAGE]", "list versions the versions DB advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new VcpkgVersionsREPL();
  if (argv.length) {

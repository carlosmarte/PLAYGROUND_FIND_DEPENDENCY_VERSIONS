 ...
 *
 *     cli.mjs   ->  CondaVersionsSDK  ->  main.mjs
 *     external  ->  CondaVersionsSDK  ->  main.mjs
 *
 ...
 * lazily provisions the isolated test conda prefix root, returns *structured*
 * results (`Report`), and is built to be:
 *
 *   * **extended** — subclass `CondaVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("numpy", { indexUrl: "conda-forge", limit: 5 });
 *     console.log(report.installable);        // ['2.1.3', ...]
 *
 *     const versions = sdk.versions("numpy");   // just list what the channel advertises
 ...
 *
 *     const s = new sdk.CondaVersionsSDK({ indexUrl: "conda-forge", condaVersion: "24.9.2" });
 *     s.config.package = "numpy";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.CondaVersionsSDK {
 ...
 *
 *     await new sdk.CondaVersionsSDK().run(["numpy", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_CONDA_VERSION = main.DEFAULT_CONDA_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use conda's default channels, no
// override; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class CondaVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "CondaVersionsError";
  }
}
 ...
 * `$CONDA_CHANNELS > $CONDA_DEFAULT_CHANNEL > conda-forge` chain. An explicit
 * `null` means "use conda's own default channels" (omit the override).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    condaVersion = DEFAULT_CONDA_VERSION, // null/"none" => skip the check
    limit = null,
    verbose = false, // stream full conda output so installs are debuggable
...
export class CondaVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._prefixRoot = null; // lazily provisioned test-prefix root
  }
   ...
   *
   * Mirrors the CLI exactly: an absent `--channel` resolves from the
   * environment (`UNSET`), and `--conda-version none` skips the conda version
   * confirmation step.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const condaVersion = String(ns.condaVersion).toLowerCase() === "none" ? null : ns.condaVersion;
    return new CondaVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The channel conda will actually receive (`null` => conda default). */
  effectiveIndexUrl() {
  ...
  /**
   * Drop the cached prefix root so the next op re-provisions it.
   *
   * Call after changing `config.venvDir` or `config.condaVersion`.
   */
  invalidateVenv() {
    this._prefixRoot = null;
  }

  /** Provision the test prefix root once and return its path. */
  ensurePip() {
    if (this._prefixRoot === null) {
      let cv = this.config.condaVersion;
      cv = cv === null || String(cv).toLowerCase() === "none" ? null : cv;
      this._prefixRoot = main.setupVenv(this.config.venvDir, cv, this.resolveEnv(), this.config.verbose);
    }
  ...
  /** List versions the channel advertises (newest-first), capped by limit. */
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new CondaVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new CondaVersionsError(`no versions found for '${name}'`);
    }

    const prefixRoot = this.ensurePip();
    const results = await main.testInstallations(
      prefixRoot, name, indexUrl, versions, this.config.output,

 ...
 *
 *     cli.mjs (this REPL)  ->  CondaVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `CondaVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions numpy
 *     node cli.mjs run numpy --limit 5 --first-only -v
 *
 * Then, at the (conda-versions) prompt:
 *     registry conda-forge
 *     versions numpy        # list what the channel advertises
 ...
 *     verbose on            # stream full conda output to debug install failures
...
import { CondaVersionsSDK, CondaVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "conda-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(conda-versions) ";

export class CondaVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new CondaVersionsSDK();
  ...
  // registry [CHANNEL]  — set or show the conda channel ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`channel = ${this.sdk.effectiveIndexUrl() || "(conda default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test prefix-root dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
    ...
    console.log(`prefix-root = ${this.cfg.venvDir}`);
  }

  // conda [VERSION|none]  — set or show the conda version expected in the test env.
  do_conda(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.condaVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`conda-version = ${this.cfg.condaVersion}`);
  }

  // verbose [on|off]  — stream full conda output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output      = ${this.cfg.output}`);
    console.log(`  prefix-root = ${this.cfg.venvDir}`);
    console.log(`  conda       = ${this.cfg.condaVersion}`);
    console.log(`  verbose     = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved conda/TLS env vars (process.env or default).
  do_env() {
  ...
  // versions [PACKAGE]  — list versions the channel advertises.
  do_versions(arg) {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof CondaVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--channel CHANNEL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ["registry [CHANNEL]", "set or show the conda channel ('none' to clear)"],
      ["index [CHANNEL]", "alias for registry"],
      ["package [NAME]", "set or show the default package"],
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test prefix-root dir"],
      ["conda [VERSION|none]", "set or show the conda version expected in the test env"],
      ["verbose [on|off]", "stream full conda output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved conda/TLS env vars"],
      ["versions [PACKAGE]", "list versions the channel advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new CondaVersionsREPL();
  if (argv.length) {

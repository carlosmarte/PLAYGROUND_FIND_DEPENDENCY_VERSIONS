 ...
 *
 *     cli.mjs (this REPL)  ->  HelmVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `HelmVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions bitnami/nginx
 *     node cli.mjs run bitnami/nginx --limit 5 --first-only -v
 *
 * Then, at the (helm-versions) prompt:
 *     registry https://charts.bitnami.com/bitnami
 *     versions bitnami/nginx   # list the versions the repo advertises
 *     find bitnami/nginx       # pull-test until the first version that works
 *     test bitnami/nginx       # pull-test every version, write a JSON report
 *     test bitnami/nginx 10    # pull-test only the newest 10 versions
 *     verbose on               # stream full helm output to debug pull failures
...
import { HelmVersionsSDK, HelmVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "helm-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(helm-versions) ";

export class HelmVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new HelmVersionsSDK();
  ...
  // registry [URL]  — set or show the chart repo URL ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`repo-url = ${this.sdk.effectiveIndexUrl() || "(helm default)"}`);
  }

  // package [NAME]  — set or show the default chart (<repo>/<chart>).
  do_package(arg) {
    arg = arg.trim();
  ...
  // venv [DIR]  — set or show the chart-pull destination dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // helm [VERSION|none]  — set or show the helm version to expect.
  do_helm(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.helmVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next pull-test
    }
    console.log(`helm-version = ${this.cfg.helmVersion}`);
  }

  // verbose [on|off]  — stream full helm output so pulls are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  helm      = ${this.cfg.helmVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved helm/TLS env vars (process.env or default).
  do_env() {
  ...
  // versions [PACKAGE]  — list versions the repo advertises.
  do_versions(arg) {
  ...
  // find [PACKAGE]  — pull-test until the first version that works.
  async do_find(arg) {
    const pkg = this._resolvePackage(arg);
    if (!pkg) return;
    try {
      ...
      if (e instanceof HelmVersionsError) console.log(e.message);
      else throw e;
    }
  }

  // test [PACKAGE] [MAX]  — pull-test versions (newest first), write report.
  async do_test(arg) {
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--repo-url URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the chart-pull destination dir"],
      ["helm [VERSION|none]", "set or show the helm version to expect"],
      ["verbose [on|off]", "stream full helm output so pulls are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved helm/TLS env vars"],
      ["versions [PACKAGE]", "list versions the repo advertises"],
      ["find [PACKAGE]", "pull-test until the first version that works"],
      ["test [PACKAGE] [MAX]", "pull-test versions, write the JSON report"],
 ...
 *
 *     docker run <image> versions bitnami/nginx      // -> REPL: `versions bitnami/nginx`
 *     docker run <image> run bitnami/nginx --limit 5 // -> batch via main.main
 *     docker run -it <image>                          // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new HelmVersionsREPL();
  if (argv.length) {

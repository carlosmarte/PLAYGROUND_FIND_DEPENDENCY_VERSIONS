 ...
 *
 *     cli.mjs   ->  HelmVersionsSDK  ->  main.mjs
 *     external  ->  HelmVersionsSDK  ->  main.mjs
 *
 ...
 * lazily provisions the isolated chart-pull destination, returns *structured*
 * results (`Report`), and is built to be:
 *
 *   * **extended** — subclass `HelmVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("bitnami/nginx", { indexUrl: "https://charts.bitnami.com/bitnami", limit: 5 });
 *     console.log(report.installable);        // ['18.2.5', ...]
 *
 *     const versions = sdk.versions("bitnami/nginx"); // just list what the repo advertises
 *     const report = await sdk.find("bitnami/nginx"); // stop at the first version that pulls
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("bitnami/nginx");
 ...
 *
 *     sdk.versionsOutput("bitnami/nginx"); // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.HelmVersionsSDK({ indexUrl: "https://charts.bitnami.com/bitnami", helmVersion: "3.16.3" });
 *     s.config.package = "bitnami/nginx";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.HelmVersionsSDK {
 ...
 *
 *     await new sdk.HelmVersionsSDK().run(["bitnami/nginx", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_HELM_VERSION = main.DEFAULT_HELM_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use helm's default repo, no
// --repo-url; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class HelmVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "HelmVersionsError";
  }
}
 ...
 * `$HELM_REPO_URL > $CHART_REGISTRY_URL > charts.helm.sh` chain. An explicit
 * `null` means "use helm's own default" (omit `--repo-url` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    helmVersion = DEFAULT_HELM_VERSION, // null/"none" => skip the check
    limit = null,
    verbose = false, // stream full helm output so pulls are debuggable
...
export class Report {
  ...
  /** Versions that pulled cleanly, newest-first. */
  get installable() {
    return this.results.filter((r) => r.status === "success").map((r) => r.version);
  }

  /** Versions that failed to pull. */
  get failed() {
    return this.results.filter((r) => r.status !== "success").map((r) => r.version);
  }
...
export class HelmVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._pipPath = null; // lazily provisioned chart-pull destination
  }
   ...
   * environment (`UNSET`), and `--helm-version none` skips the binary-version
   * check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const helmVersion = String(ns.helmVersion).toLowerCase() === "none" ? null : ns.helmVersion;
    return new HelmVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The repo URL helm will actually receive (`null` => helm default). */
  effectiveIndexUrl() {
  ...
  /**
   * Drop the cached destination path so the next op re-provisions it.
   *
   * Call after changing `config.venvDir` or `config.helmVersion`.
   */
  invalidateVenv() {
    this._pipPath = null;
  }

  /** Provision the chart-pull destination once and return its path. */
  ensurePip() {
    if (this._pipPath === null) {
      let hv = this.config.helmVersion;
      hv = hv === null || String(hv).toLowerCase() === "none" ? null : hv;
      this._pipPath = main.setupVenv(this.config.venvDir, hv, this.resolveEnv(), this.config.verbose);
    }
  ...
  /** Pull-test until the first version that works; return a `Report`. */
  find(pkg = null) {
    return this._probe(pkg, UNSET, true);
  }

  /**
   * Pull-test versions (newest-first), write the JSON report, return it.
   *
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new HelmVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new HelmVersionsError(`no versions found for '${name}'`);
    }

    const destPath = this.ensurePip();
    const results = await main.testInstallations(
      destPath, name, indexUrl, versions, this.config.output,
...
export function versions(pkg, config = {}) {
  return new HelmVersionsSDK({ package: pkg, ...config }).availableVersions();
}

/** One-shot: JSON-able envelope of the versions a registry advertises. */
export function versionsOutput(pkg, config = {}) {
  return new HelmVersionsSDK({ package: pkg, ...config }).versionsOutput();
}

/** One-shot: pull-test until the first version that works. */
export function find(pkg, config = {}) {
  return new HelmVersionsSDK({ package: pkg, ...config }).find();
}

/** One-shot: pull-test versions and write a report. */
export function test(pkg, config = {}) {
  return new HelmVersionsSDK({ package: pkg, ...config }).test();
}

/** One-shot raw passthrough to `main.main`. */
export function run(argv) {
  return new HelmVersionsSDK().run(argv);
}

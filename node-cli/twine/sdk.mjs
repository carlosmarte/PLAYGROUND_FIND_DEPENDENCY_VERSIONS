 ...
 *
 *     cli.mjs   ->  TwineVersionsSDK  ->  main.mjs
 *     external  ->  TwineVersionsSDK  ->  main.mjs
 *
 ...
 *
 *     const s = new sdk.TwineVersionsSDK({ indexUrl: "https://reg/simple", twineVersion: "5.1.1" });
 *     s.config.package = "numpy";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.TwineVersionsSDK {
 ...
 *
 *     await new sdk.TwineVersionsSDK().run(["numpy", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_TWINE_VERSION = main.DEFAULT_TWINE_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use the default source, no
// --index-url; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class TwineVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "TwineVersionsError";
  }
}
 ...
 * explicit `null` means "use the default index" (omit `--index-url`
 * entirely).
 */
export class Config {
  constructor({
    ...
    twineVersion = DEFAULT_TWINE_VERSION, // null/"none" => keep installed twine
    limit = null,
    verbose = false, // stream full pip/twine output so checks are debuggable
    ...
    this.output = output;
    this.twineVersion = twineVersion;
    this.limit = limit;
...
/** Structured outcome of a metadata-check run (wraps `main`'s raw objects). */
export class Report {
...
export class TwineVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._scratchDir = null; // lazily provisioned scratch download dir
  }
   ...
   * environment (`UNSET`), and `--twine-version none` keeps whatever twine
   * is on PATH.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const twineVersion = String(ns.twineVersion).toLowerCase() === "none" ? null : ns.twineVersion;
    return new TwineVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The index URL pip will actually receive (`null` => default index). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.twineVersion`.
   */
  invalidateVenv() {
  ...
  ensurePip() {
    if (this._scratchDir === null) {
      let tv = this.config.twineVersion;
      tv = tv === null || String(tv).toLowerCase() === "none" ? null : tv;
      this._scratchDir = main.setupVenv(this.config.venvDir, tv, this.resolveEnv(), this.config.verbose);
    }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new TwineVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new TwineVersionsError(`no versions found for '${name}'`);
    }

    const scratchDir = this.ensurePip();
    const results = await main.testInstallations(
      scratchDir, name, indexUrl, versions, this.config.output,

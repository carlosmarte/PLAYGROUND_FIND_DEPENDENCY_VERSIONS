 ...
 *
 *     cli.mjs (this REPL)  ->  TwineVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `TwineVersionsSDK` directly.
 *
...
import { TwineVersionsSDK, TwineVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const PROMPT = "(twine-versions) ";

export class TwineVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new TwineVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`index-url = ${this.sdk.effectiveIndexUrl() || "(default index)"}`);
  }
  ...
  do_twine(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.twineVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next metadata-test
    }
    console.log(`twine-version = ${this.cfg.twineVersion}`);
  }
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  twine     = ${this.cfg.twineVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved twine/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof TwineVersionsError) console.log(e.message);
      else throw e;
    }
  }

  // test [PACKAGE] [MAX]  — metadata-check versions (newest first), write report.
  async do_test(arg) {
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the scratch download dir"],
      ["twine [VERSION|none]", "set or show the twine version expected for the probe"],
      ["verbose [on|off]", "stream full pip/twine output so checks are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved twine/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "check until the first version whose metadata validates"],
      ["test [PACKAGE] [MAX]", "metadata-check versions, write the JSON report"],
...
export async function main(argv = null) {
  ...
  const repl = new TwineVersionsREPL();
  if (argv.length) {

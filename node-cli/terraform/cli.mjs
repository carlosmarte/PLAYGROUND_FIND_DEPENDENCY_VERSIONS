 ...
 *
 *     cli.mjs (this REPL)  ->  TerraformVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `TerraformVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions hashicorp/aws
 *     node cli.mjs run hashicorp/aws --limit 5 --first-only -v
 *
 * Then, at the (terraform-versions) prompt:
 *     registry registry.terraform.io
 *     versions hashicorp/aws   # list the versions the registry advertises
 *     find hashicorp/aws       # init-test until the first version that works
 *     test hashicorp/aws       # init-test every version, write a JSON report
 *     test hashicorp/aws 10    # init-test only the newest 10 versions
 *     verbose on               # stream full terraform output to debug init failures
...
import { TerraformVersionsSDK, TerraformVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "terraform-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(terraform-versions) ";

export class TerraformVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new TerraformVersionsSDK();
  ...
  // registry [HOST]  — set or show the registry host ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`registry = ${this.sdk.effectiveIndexUrl() || "(terraform default)"}`);
  }

  // package [NAME]  — set or show the default provider (<namespace>/<name>).
  do_package(arg) {
    arg = arg.trim();
  ...
  // venv [DIR]  — set or show the init sandbox dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // terraform [VERSION|none]  — set or show the terraform version to expect.
  do_terraform(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.terraformVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next init-test
    }
    console.log(`terraform-version = ${this.cfg.terraformVersion}`);
  }

  // verbose [on|off]  — stream full terraform output so inits are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  terraform = ${this.cfg.terraformVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved terraform/TLS env vars (process.env or default).
  do_env() {
  ...
  // find [PACKAGE]  — init-test until the first version that works.
  async do_find(arg) {
    const pkg = this._resolvePackage(arg);
    if (!pkg) return;
    try {
      ...
      if (e instanceof TerraformVersionsError) console.log(e.message);
      else throw e;
    }
  }

  // test [PACKAGE] [MAX]  — init-test versions (newest first), write report.
  async do_test(arg) {
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--registry HOST] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ["registry [HOST]", "set or show the registry host ('none' to clear)"],
      ["index [HOST]", "alias for registry"],
      ["package [NAME]", "set or show the default provider (<namespace>/<name>)"],
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the init sandbox dir"],
      ["terraform [VERSION|none]", "set or show the terraform version to expect"],
      ["verbose [on|off]", "stream full terraform output so inits are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved terraform/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "init-test until the first version that works"],
      ["test [PACKAGE] [MAX]", "init-test versions, write the JSON report"],
    ...
    for (const [name, desc] of docs) console.log(`  ${name.padEnd(26)} ${desc}`);
  }
 ...
 *
 *     docker run <image> versions hashicorp/aws    // -> REPL: `versions hashicorp/aws`
 *     docker run <image> run hashicorp/aws --limit 5 // -> batch via main.main
 *     docker run -it <image>                       // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new TerraformVersionsREPL();
  if (argv.length) {

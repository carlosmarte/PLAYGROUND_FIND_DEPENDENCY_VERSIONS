 ...
 *
 *     cli.mjs (this REPL)  ->  StackVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `StackVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions aeson
 *     node cli.mjs run aeson --limit 5 --first-only -v
 *
 * Then, at the (stack-versions) prompt:
 *     registry https://my-registry.example.com
 *     versions aeson        # list what the registry advertises
 *     find aeson            # resolve-test until the first version that works
 *     test aeson            # resolve-test every version, write a JSON report
 *     test aeson 10         # resolve-test only the newest 10 versions
 *     verbose on            # stream full stack output to debug resolve failures
...
import { StackVersionsSDK, StackVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "stack-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(stack-versions) ";

export class StackVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new StackVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`index-url = ${this.sdk.effectiveIndexUrl() || "(stack default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test resolve sandbox dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // stack [VERSION|none]  — set or show the stack version expected in the sandbox.
  do_stack(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.stackVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next resolve-test
    }
    console.log(`stack-version = ${this.cfg.stackVersion}`);
  }

  // verbose [on|off]  — stream full stack output so resolves are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  stack     = ${this.cfg.stackVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved stack/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof StackVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test resolve sandbox dir"],
      ["stack [VERSION|none]", "set or show the stack version expected in the sandbox"],
      ["verbose [on|off]", "stream full stack output so resolves are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved stack/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "resolve-test until the first version that works"],
 ...
 *
 *     docker run <image> versions aeson          // -> REPL: `versions aeson`
 *     docker run <image> run aeson --limit 5     // -> batch via main.main
 *     docker run -it <image>                     // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new StackVersionsREPL();
  if (argv.length) {

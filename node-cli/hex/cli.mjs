 ...
 *
 *     cli.mjs (this REPL)  ->  HexVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `HexVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions jason
 *     node cli.mjs run jason --limit 5 --first-only -v
 *
 * Then, at the (hex-versions) prompt:
 *     registry https://my-registry.example.com
 *     versions jason        # list what the registry advertises
 *     find jason            # fetch-test until the first version that works
 *     test jason            # fetch-test every version, write a JSON report
 *     test jason 10         # fetch-test only the newest 10 versions
 *     verbose on            # stream full mix output to debug fetch failures
...
import { HexVersionsSDK, HexVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "hex-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(hex-versions) ";

export class HexVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new HexVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`index-url = ${this.sdk.effectiveIndexUrl() || "(hex default)"}`);
  }
  ...
  // hex [VERSION|none]  — set or show the hex version expected in the sandbox.
  do_hex(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.hexVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next fetch-test
    }
    console.log(`hex-version = ${this.cfg.hexVersion}`);
  }

  // verbose [on|off]  — stream full mix output so fetches are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  hex       = ${this.cfg.hexVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved hex/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof HexVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["verbose [on|off]", "stream full mix output so fetches are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved hex/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "fetch-test until the first version that works"],
 ...
 *
 *     docker run <image> versions jason        // -> REPL: `versions jason`
 *     docker run <image> run jason --limit 5   // -> batch via main.main
 *     docker run -it <image>                    // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new HexVersionsREPL();
  if (argv.length) {

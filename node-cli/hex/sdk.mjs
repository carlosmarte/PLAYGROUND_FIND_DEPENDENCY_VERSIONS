 ...
 *
 *     cli.mjs   ->  HexVersionsSDK  ->  main.mjs
 *     external  ->  HexVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `HexVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("jason", { indexUrl: "https://hex.pm", limit: 5 });
 *     console.log(report.installable);        // ['1.4.4', ...]
 *
 *     const versions = await sdk.versions("jason"); // just list what the registry advertises
 *     const report = await sdk.find("jason");       // stop at the first version that fetches
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("jason");
 ...
 *
 *     sdk.versionsOutput("jason");        // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.HexVersionsSDK({ indexUrl: "https://hex.pm", hexVersion: "2.1.1" });
 *     s.config.package = "jason";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.HexVersionsSDK {
 ...
 *
 *     await new sdk.HexVersionsSDK().run(["jason", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_HEX_VERSION = main.DEFAULT_HEX_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use hex's default, no
// --index-url; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class HexVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "HexVersionsError";
  }
}
 ...
 * `$HEX_REGISTRY_URL > $HEX_API_URL > hex.pm` chain. An explicit `null` means
 * "use hex's own default" (omit `--index-url` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    hexVersion = DEFAULT_HEX_VERSION, // null/"none" => skip version check
    limit = null,
    verbose = false, // stream full mix output so fetches are debuggable
...
export class HexVersionsSDK {
   ...
   * environment (`UNSET`), and `--hex-version none` skips the sandbox version
   * check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const hexVersion = String(ns.hexVersion).toLowerCase() === "none" ? null : ns.hexVersion;
    return new HexVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The index URL hex will actually receive (`null` => hex default). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.hexVersion`.
   */
  invalidateVenv() {
    this._sandbox = null;
  }
  ...
  ensurePip() {
    if (this._sandbox === null) {
      let hv = this.config.hexVersion;
      hv = hv === null || String(hv).toLowerCase() === "none" ? null : hv;
      this._sandbox = main.setupVenv(this.config.venvDir, hv, this.resolveEnv(), this.config.verbose);
    }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new HexVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new HexVersionsError(`no versions found for '${name}'`);
    }

 ...
 *
 *     cli.mjs   ->  AptVersionsSDK  ->  main.mjs
 *     external  ->  AptVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `AptVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("bash", { indexUrl: "http://deb.debian.org/debian", limit: 5 });
 *     console.log(report.installable);        // ['5.2.15-2+b2', ...]
 *
 ...
 *
 *     const s = new sdk.AptVersionsSDK({ indexUrl: "http://reg/debian", aptVersion: "2.6.1" });
 *     s.config.package = "bash";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.AptVersionsSDK {
 ...
 *
 *     await new sdk.AptVersionsSDK().run(["bash", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_APT_VERSION = main.DEFAULT_APT_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use apt's default, no
// --repository; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class AptVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "AptVersionsError";
  }
}
 ...
 * `null` means "use apt's own default" (omit `--repository` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    aptVersion = DEFAULT_APT_VERSION, // null/"none" => skip the version check
    limit = null,
    verbose = false, // stream full apt output so installs are debuggable
...
export class AptVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._cachePath = null; // lazily provisioned test apt cache
  }
   ...
   * environment (`UNSET`), and `--apt-version none` skips the host apt version
   * check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const aptVersion = String(ns.aptVersion).toLowerCase() === "none" ? null : ns.aptVersion;
    return new AptVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The repo URL apt will actually receive (`null` => apt default). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.aptVersion`.
   */
  invalidateVenv() {
  ...
  ensurePip() {
    if (this._cachePath === null) {
      let av = this.config.aptVersion;
      av = av === null || String(av).toLowerCase() === "none" ? null : av;
      this._cachePath = main.setupVenv(this.config.venvDir, av, this.resolveEnv(), this.config.verbose);
    }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new AptVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new AptVersionsError(`no versions found for '${name}'`);
    }

 ...
 *
 *     cli.mjs (this REPL)  ->  AptVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `AptVersionsSDK` directly.
 *
 ...
 *
 * Then, at the (apt-versions) prompt:
 ...
 *     verbose on            # stream full apt output to debug install failures
...
import { AptVersionsSDK, AptVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "apt-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(apt-versions) ";

export class AptVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new AptVersionsSDK();
  ...
  // registry [URL]  — set or show the apt repository URL ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`repository = ${this.sdk.effectiveIndexUrl() || "(apt default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test apt cache dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // apt [VERSION|none]  — set or show the apt version expected in the test env.
  do_apt(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.aptVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`apt-version = ${this.cfg.aptVersion}`);
  }

  // verbose [on|off]  — stream full apt output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output     = ${this.cfg.output}`);
    console.log(`  venv-dir   = ${this.cfg.venvDir}`);
    console.log(`  apt        = ${this.cfg.aptVersion}`);
    console.log(`  verbose    = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved apt/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof AptVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test apt cache dir"],
      ["apt [VERSION|none]", "set or show the apt version expected in the test env"],
      ["verbose [on|off]", "stream full apt output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved apt/TLS env vars"],
      ["versions [PACKAGE]", "list versions the repository advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new AptVersionsREPL();
  if (argv.length) {

 ...
 *
 *     cli.mjs (this REPL)  ->  PipVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `PipVersionsSDK` directly.
 *
...
import { PipVersionsSDK, PipVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const PROMPT = "(pip-versions) ";

export class PipVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new PipVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`index-url = ${this.sdk.effectiveIndexUrl() || "(pip default)"}`);
  }
  ...
  do_pip(arg) {
    ...
    console.log(`pip-version = ${this.cfg.pipVersion}`);
  }
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  pip       = ${this.cfg.pipVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved pip/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof PipVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test virtualenv dir"],
      ["pip [VERSION|none]", "set or show the pip version pinned in the test venv"],
      ["verbose [on|off]", "stream full pip output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved pip/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new PipVersionsREPL();
  if (argv.length) {

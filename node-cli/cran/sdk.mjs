 ...
 *
 *     cli.mjs   ->  CranVersionsSDK  ->  main.mjs
 *     external  ->  CranVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `CranVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("jsonlite", { indexUrl: "https://cloud.r-project.org", limit: 5 });
 *     console.log(report.installable);        // ['1.8.8', ...]
 *
 *     const versions = await sdk.versions("jsonlite"); // just list what the registry advertises
 *     const report = await sdk.find("jsonlite");       // stop at the first version that installs
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("jsonlite");
 ...
 *
 *     sdk.versionsOutput("jsonlite");        // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.CranVersionsSDK({ indexUrl: "https://cloud.r-project.org", rVersion: "4.4.2" });
 *     s.config.package = "jsonlite";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.CranVersionsSDK {
 ...
 *
 *     await new sdk.CranVersionsSDK().run(["jsonlite", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_R_VERSION = main.DEFAULT_R_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use R's default repos, no
// --repos override; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class CranVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "CranVersionsError";
  }
}
 ...
 * `null` means "use R's own default repos" (omit the `--repos` override).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    rVersion = DEFAULT_R_VERSION, // null/"none" => skip the version check
    limit = null,
    verbose = false, // stream full R output so installs are debuggable
...
export class CranVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._libPath = null; // lazily provisioned test-library path
  }
   ...
   * environment (`UNSET`), and `--r-version none` skips the R version
   * confirmation step.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const rVersion = String(ns.rVersion).toLowerCase() === "none" ? null : ns.rVersion;
    return new CranVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The repos URL R will actually receive (`null` => R default). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.rVersion`.
   */
  invalidateVenv() {
    this._libPath = null;
  }
  ...
  ensurePip() {
    if (this._libPath === null) {
      let rv = this.config.rVersion;
      rv = rv === null || String(rv).toLowerCase() === "none" ? null : rv;
      this._libPath = main.setupVenv(this.config.venvDir, rv, this.resolveEnv(), this.config.verbose);
    }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new CranVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new CranVersionsError(`no versions found for '${name}'`);
    }

    const libPath = this.ensurePip();
    const results = await main.testInstallations(
      libPath, name, indexUrl, versions, this.config.output,

 ...
 *
 *     cli.mjs (this REPL)  ->  CranVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `CranVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions jsonlite
 *     node cli.mjs run jsonlite --limit 5 --first-only -v
 *
 * Then, at the (cran-versions) prompt:
 *     registry https://cloud.r-project.org
 *     versions jsonlite     # list what the registry advertises
 *     find jsonlite         # install-test until the first version that works
 *     test jsonlite         # install-test every version, write a JSON report
 *     test jsonlite 10      # install-test only the newest 10 versions
 *     verbose on            # stream full R output to debug install failures
...
import { CranVersionsSDK, CranVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "cran-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(cran-versions) ";

export class CranVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new CranVersionsSDK();
  ...
  // registry [URL]  — set or show the repos URL ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`repos = ${this.sdk.effectiveIndexUrl() || "(R default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test R library dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
    ...
    console.log(`lib-dir = ${this.cfg.venvDir}`);
  }

  // r [VERSION|none]  — set or show the R version expected in the test library.
  do_r(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.rVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`r-version = ${this.cfg.rVersion}`);
  }

  // verbose [on|off]  — stream full R output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  lib-dir   = ${this.cfg.venvDir}`);
    console.log(`  r         = ${this.cfg.rVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved R/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof CranVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--repos URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test R library dir"],
      ["r [VERSION|none]", "set or show the R version expected in the test library"],
      ["verbose [on|off]", "stream full R output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved R/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions jsonlite      // -> REPL: `versions jsonlite`
 *     docker run <image> run jsonlite --limit 5 // -> batch via main.main
 *     docker run -it <image>                     // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new CranVersionsREPL();
  if (argv.length) {

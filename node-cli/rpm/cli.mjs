 ...
 *
 *     cli.mjs (this REPL)  ->  RpmVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `RpmVersionsSDK` directly.
 *
 ...
 *
 * Then, at the (rpm-versions) prompt:
 *     registry fedora
 ...
 *     verbose on            # stream full dnf output to debug install failures
...
import { RpmVersionsSDK, RpmVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "rpm-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(rpm-versions) ";

export class RpmVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new RpmVersionsSDK();
  ...
  // registry [REPO]  — set or show the dnf repo id ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`repository = ${this.sdk.effectiveIndexUrl() || "(dnf default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test download dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // dnf [VERSION|none]  — set or show the dnf version expected in the test env.
  do_dnf(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.dnfVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`dnf-version = ${this.cfg.dnfVersion}`);
  }

  // verbose [on|off]  — stream full dnf output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output     = ${this.cfg.output}`);
    console.log(`  venv-dir   = ${this.cfg.venvDir}`);
    console.log(`  dnf        = ${this.cfg.dnfVersion}`);
    console.log(`  verbose    = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved dnf/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof RpmVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--repository REPO] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ["registry [REPO]", "set or show the dnf repo id ('none' to clear)"],
      ["index [REPO]", "alias for registry"],
      ["package [NAME]", "set or show the default package"],
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test download dir"],
      ["dnf [VERSION|none]", "set or show the dnf version expected in the test env"],
      ["verbose [on|off]", "stream full dnf output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved dnf/TLS env vars"],
      ["versions [PACKAGE]", "list versions the repository advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new RpmVersionsREPL();
  if (argv.length) {

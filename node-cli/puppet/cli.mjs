 ...
 *
 *     cli.mjs (this REPL)  ->  PuppetVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `PuppetVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions puppetlabs-stdlib
 *     node cli.mjs run puppetlabs-stdlib --limit 5 --first-only -v
 *
 * Then, at the (puppet-versions) prompt:
 *     registry https://my-forge.example.com   # set the Forge server
 *     versions puppetlabs-stdlib    # list what the Forge advertises
 *     find puppetlabs-stdlib        # install-test until the first version that works
 *     test puppetlabs-stdlib        # install-test every version, write a JSON report
 *     test puppetlabs-stdlib 10     # install-test only the newest 10 versions
 *     verbose on                    # stream full puppet output to debug failures
...
import { PuppetVersionsSDK, PuppetVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "puppet-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(puppet-versions) ";

export class PuppetVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new PuppetVersionsSDK();
  ...
  // registry [URL]  — set or show the Forge server ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.forgeServer = arg.toLowerCase() === "none" ? null : arg;
    console.log(`forge-server = ${this.sdk.effectiveForgeServer() || "(puppet default)"}`);
  }

  // package [NAME]  — set or show the default module (user-mod).
  do_package(arg) {
    arg = arg.trim();
  ...
  // venv [DIR]  — set or show the sandbox target dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // puppet [VERSION|none]  — set or show the puppet version expected in the test env.
  do_puppet(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.puppetVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`puppet-version = ${this.cfg.puppetVersion}`);
  }

  // verbose [on|off]  — stream full puppet output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output       = ${this.cfg.output}`);
    console.log(`  venv-dir     = ${this.cfg.venvDir}`);
    console.log(`  puppet       = ${this.cfg.puppetVersion}`);
    console.log(`  verbose      = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved puppet/TLS env vars (process.env or default).
  do_env() {
  ...
  // versions [PACKAGE]  — list versions the Forge advertises.
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof PuppetVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--forge-server URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the sandbox target dir"],
      ["puppet [VERSION|none]", "set or show the puppet version expected in the test env"],
      ["verbose [on|off]", "stream full puppet output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved puppet/TLS env vars"],
      ["versions [PACKAGE]", "list versions the Forge advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions puppetlabs-stdlib   // -> REPL: `versions ...`
 *     docker run <image> run puppetlabs-stdlib --limit 5 // -> batch via main.main
 *     docker run -it <image>                          // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new PuppetVersionsREPL();
  if (argv.length) {

 ...
 *
 *     cli.mjs (this REPL)  ->  GradleVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `GradleVersionsSDK` directly.
 *
...
import { GradleVersionsSDK, GradleVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const PROMPT = "(gradle-versions) ";

export class GradleVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new GradleVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`repo-url = ${this.sdk.effectiveIndexUrl() || "(gradle default)"}`);
  }
  ...
  do_gradle(arg) {
    ...
    console.log(`gradle-version = ${this.cfg.pipVersion}`);
  }
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  gradle    = ${this.cfg.pipVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved gradle/HTTP env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof GradleVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the Gradle sandbox dir"],
      ["gradle [VERSION|none]", "set or show the gradle version verified in the test env"],
      ["verbose [on|off]", "stream full gradle output so resolves are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved gradle/HTTP env vars"],
      ["versions [PACKAGE]", "list versions the repository advertises"],
      ["find [PACKAGE]", "resolve-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new GradleVersionsREPL();
  if (argv.length) {

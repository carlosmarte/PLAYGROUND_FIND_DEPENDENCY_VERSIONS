 ...
 *
 *     cli.mjs   ->  WingetVersionsSDK  ->  main.mjs
 *     external  ->  WingetVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `WingetVersionsSDK` and override the `beforeProbe`
 ...
 *
 * NOTE: winget is Windows-only — every operation shells out to the `winget` CLI,
 * which only exists on Windows with it on PATH. On other hosts these calls fail.
 *
 * External one-liners:
 *
 *     import * as sdk from "./sdk.mjs";
 *     const report = await sdk.test("Git.Git", { source: "winget", limit: 5 });
 *     console.log(report.installable);        // ['2.45.0', ...]
 *
 *     const versions = sdk.versions("Git.Git"); // just list what the source advertises
 *     const report = await sdk.find("Git.Git"); // stop at the first version that installs
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("Git.Git");
 ...
 *
 *     sdk.versionsOutput("Git.Git");      // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.WingetVersionsSDK({ source: "winget", wingetVersion: "1.8.0" });
 *     s.config.package = "Git.Git";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.WingetVersionsSDK {
 ...
 *
 *     await new sdk.WingetVersionsSDK().run(["Git.Git", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_WINGET_VERSION = main.DEFAULT_WINGET_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use winget's default, no
// --source; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class WingetVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "WingetVersionsError";
  }
}
 ...
 * `$WINGET_SOURCE > $WINGET_REGISTRY_URL > winget` chain. An explicit `null`
 * means "use winget's own default" (omit `--source` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    wingetVersion = DEFAULT_WINGET_VERSION, // null/"none" => skip toolchain check
    limit = null,
    verbose = false, // stream full winget output so installs are debuggable
...
export class WingetVersionsSDK {
  constructor(config = null) {
    ...
    this._venvDir = null; // lazily provisioned test-sandbox dir
  }
   ...
   *
   * Mirrors the CLI exactly: an absent `--source` resolves from the environment
   * (`UNSET`), and `--winget-version none` skips the toolchain check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const wingetVersion = String(ns.wingetVersion).toLowerCase() === "none" ? null : ns.wingetVersion;
    return new WingetVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The source winget will actually receive (`null` => its default). */
  effectiveIndexUrl() {
  ...
  /**
   * Drop the cached test sandbox so the next op re-provisions it.
   *
   * Call after changing `config.venvDir` or `config.wingetVersion`.
   */
  invalidateVenv() {
    this._venvDir = null;
  }

  /** Provision the test sandbox once and return its directory. */
  ensurePip() {
    if (this._venvDir === null) {
      let wv = this.config.wingetVersion;
      wv = wv === null || String(wv).toLowerCase() === "none" ? null : wv;
      this._venvDir = main.setupVenv(this.config.venvDir, wv, this.resolveEnv(), this.config.verbose);
    }
  ...
  /** List versions the source advertises (newest-first), capped by limit. */
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new WingetVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new WingetVersionsError(`no versions found for '${name}'`);
    }

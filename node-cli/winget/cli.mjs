 ...
 *
 *     cli.mjs (this REPL)  ->  WingetVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `WingetVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions Git.Git
 *     node cli.mjs run Git.Git --limit 5 --first-only -v
 *
 * Then, at the (winget-versions) prompt:
 *     registry winget            # set the source name
 *     versions Git.Git           # list what the source advertises
 *     find Git.Git               # install-test until the first version that works
 *     test Git.Git               # install-test every version, write a JSON report
 *     test Git.Git 10            # install-test only the newest 10 versions
 *     verbose on                 # stream full winget output to debug install failures
...
import { WingetVersionsSDK, WingetVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "winget-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(winget-versions) ";

export class WingetVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new WingetVersionsSDK();
  ...
  // registry [SOURCE]  — set or show the source name ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`source = ${this.sdk.effectiveIndexUrl() || "(winget default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test sandbox dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // winget [VERSION|none]  — set or show the winget version the test env checks.
  do_winget(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.wingetVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`winget-version = ${this.cfg.wingetVersion}`);
  }

  // verbose [on|off]  — stream full winget output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  winget    = ${this.cfg.wingetVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved winget/TLS env vars (process.env or default).
  do_env() {
  ...
  // versions [PACKAGE]  — list versions the source advertises.
  do_versions(arg) {
    const pkg = this._resolvePackage(arg);
    if (!pkg) return;
    const payload = this.sdk.versionsOutput(pkg);
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof WingetVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--source NAME] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ["registry [SOURCE]", "set or show the source name ('none' to clear)"],
      ["index [SOURCE]", "alias for registry"],
      ["package [NAME]", "set or show the default package"],
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test sandbox dir"],
      ["winget [VERSION|none]", "set or show the winget version the test env checks"],
      ["verbose [on|off]", "stream full winget output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved winget/TLS env vars"],
      ["versions [PACKAGE]", "list versions the source advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions Git.Git       // -> REPL: `versions ...`
 *     docker run <image> run Git.Git --limit 5  // -> batch via main.main
 *     docker run -it <image>                    // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new WingetVersionsREPL();
  if (argv.length) {

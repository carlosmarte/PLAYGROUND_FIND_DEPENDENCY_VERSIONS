 ...
 *
 *     cli.mjs (this REPL)  ->  ClojarsVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `ClojarsVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions org.clojure:clojure
 *     node cli.mjs run org.clojure:clojure --limit 5 --first-only -v
 *
 * Then, at the (clojars-versions) prompt:
 *     registry https://repo.clojars.org
 *     versions org.clojure:clojure   # list what Clojars advertises
 *     find org.clojure:clojure       # resolve-test until the first version that works
 *     test org.clojure:clojure       # resolve-test every version, write a JSON report
 *     test org.clojure:clojure 10    # resolve-test only the newest 10 versions
 *     verbose on            # stream full mvn output to debug resolution failures
...
import { ClojarsVersionsSDK, ClojarsVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "clojars-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(clojars-versions) ";

export class ClojarsVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new ClojarsVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`repo-url = ${this.sdk.effectiveIndexUrl() || "(clojars default)"}`);
  }
  ...
  // venv [DIR]  — set or show the local Maven repo dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // clojure [VERSION|none]  — set or show the clojure version verified in the test env.
  do_clojure(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.pipVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next resolve-test
    }
    console.log(`clojure-version = ${this.cfg.pipVersion}`);
  }

  // verbose [on|off]  — stream full mvn output so resolves are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  clojure   = ${this.cfg.pipVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved mvn/HTTP env vars (process.env or default).
  do_env() {
  ...
  // versions [PACKAGE]  — list versions Clojars advertises.
  do_versions(arg) {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof ClojarsVersionsError) console.log(e.message);
      else throw e;
    }
  }

  // test [PACKAGE] [MAX]  — resolve-test versions (newest first), write report.
  async do_test(arg) {
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the local Maven repo dir"],
      ["clojure [VERSION|none]", "set or show the clojure version verified in the test env"],
      ["verbose [on|off]", "stream full mvn output so resolves are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved mvn/HTTP env vars"],
      ["versions [PACKAGE]", "list versions Clojars advertises"],
      ["find [PACKAGE]", "resolve-test until the first version that works"],
      ["test [PACKAGE] [MAX]", "resolve-test versions, write the JSON report"],
 ...
 *
 *     docker run <image> versions org.clojure:clojure   // -> REPL: `versions ...`
 *     docker run <image> run org.clojure:clojure --limit 5 // -> batch via main.main
 *     docker run -it <image>                   // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new ClojarsVersionsREPL();
  if (argv.length) {

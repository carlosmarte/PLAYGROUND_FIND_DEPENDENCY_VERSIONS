 ...
 *
 *     cli.mjs   ->  ClojarsVersionsSDK  ->  main.mjs
 *     external  ->  ClojarsVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `ClojarsVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("org.clojure:clojure", { indexUrl: "https://repo.clojars.org", limit: 5 });
 *     console.log(report.installable);        // ['1.12.0', ...]
 *
 *     const versions = await sdk.versions("org.clojure:clojure"); // just list what Clojars advertises
 *     const report = await sdk.find("org.clojure:clojure");       // stop at the first version that resolves
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("org.clojure:clojure");
 ...
 *
 *     sdk.versionsOutput("org.clojure:clojure"); // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.ClojarsVersionsSDK({ indexUrl: "https://repo.clojars.org", pipVersion: "1.12.0" });
 *     s.config.package = "org.clojure:clojure";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.ClojarsVersionsSDK {
 ...
 *
 *     await new sdk.ClojarsVersionsSDK().run(["org.clojure:clojure", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_CLOJURE_VERSION = main.DEFAULT_CLOJURE_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use the resolver's default, no
// --repo-url; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class ClojarsVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "ClojarsVersionsError";
  }
}
 ...
 * `$CLOJARS_REPO_URL > $JVM_REGISTRY_URL > repo.clojars.org` chain. An explicit
 * `null` means "use the resolver's own default" (omit the repo).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    pipVersion = DEFAULT_CLOJURE_VERSION, // null/"none" => skip the version check
    limit = null,
    verbose = false, // stream full mvn output so resolves are debuggable
...
export class ClojarsVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._pipPath = null; // lazily provisioned local Maven repository path
  }
   ...
   * environment (`UNSET`), and `--clojure-version none` skips the version check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const clojureVersion = String(ns.clojureVersion).toLowerCase() === "none" ? null : ns.clojureVersion;
    return new ClojarsVersionsSDK(new Config({
      package: ns.package,
      indexUrl: ns.repoUrl === null ? UNSET : ns.repoUrl,
      venvDir: ns.venvDir,
      output: ns.output,
      pipVersion: clojureVersion,
      limit: ns.limit,
      verbose: ns.verbose,
    }));
  }
  ...
  /** The repo URL mvn will actually receive (`null` => resolver default). */
  effectiveIndexUrl() {
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new ClojarsVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new ClojarsVersionsError(`no versions found for '${name}'`);
    }

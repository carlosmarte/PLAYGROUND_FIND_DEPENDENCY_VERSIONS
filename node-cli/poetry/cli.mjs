 ...
 *
 *     cli.mjs (this REPL)  ->  PoetryVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `PoetryVersionsSDK` directly.
 *
 ...
 *
 * Then, at the (poetry-versions) prompt:
 ...
 *     verbose on            # stream full poetry output to debug install failures
...
import { PoetryVersionsSDK, PoetryVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "poetry-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(poetry-versions) ";

export class PoetryVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new PoetryVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`index-url = ${this.sdk.effectiveIndexUrl() || "(poetry default)"}`);
  }
  ...
  // poetry [VERSION|none]  — set or show the poetry version expected for the test project.
  do_poetry(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.poetryVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`poetry-version = ${this.cfg.poetryVersion}`);
  }

  // verbose [on|off]  — stream full poetry output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  poetry    = ${this.cfg.poetryVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved poetry/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof PoetryVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["verbose [on|off]", "stream full poetry output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved poetry/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new PoetryVersionsREPL();
  if (argv.length) {

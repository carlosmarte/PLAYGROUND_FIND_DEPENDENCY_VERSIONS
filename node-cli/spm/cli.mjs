 ...
 *
 *     cli.mjs (this REPL)  ->  SpmVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `SpmVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions https://github.com/apple/swift-argument-parser.git
 *     node cli.mjs run https://github.com/apple/swift-argument-parser.git --limit 5 --first-only -v
 *
...
import { SpmVersionsSDK, SpmVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const PROMPT = "(spm-versions) ";

export class SpmVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new SpmVersionsSDK();
  ...
  _resolvePackage(arg) {
    ...
    if (!pkg) {
      console.log("No package set. Use 'package <repo-url>' or pass one inline.");
    } else if (arg.trim()) {
  ...
  do_registry(arg) {
    ...
    console.log(`registry = ${this.sdk.effectiveIndexUrl() || "(host default)"}`);
  }
  ...
  do_swift(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.swiftVersion = arg;
      this.sdk.invalidateVenv(); // re-assert on next resolve-test
    }
    console.log(`swift-version = ${this.cfg.swiftVersion}`);
  }
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  swift     = ${this.cfg.swiftVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved swift/git/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof SpmVersionsError) console.log(e.message);
      else throw e;
    }
  }

  // test [REPO-URL] [MAX]  — resolve-test versions (newest first), write report.
  async do_test(arg) {
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <repo-url> [--registry URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the throwaway test package dir"],
      ["swift [VERSION|none]", "set or show the swift version asserted in the test package"],
      ["verbose [on|off]", "stream full swift output so resolves are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved swift/git/TLS env vars"],
      ["versions [REPO-URL]", "list the semver tags the repo advertises"],
      ["find [REPO-URL]", "resolve-test until the first version that works"],
      ["test [REPO-URL] [MAX]", "resolve-test versions, write the JSON report"],
...
export async function main(argv = null) {
  ...
  const repl = new SpmVersionsREPL();
  if (argv.length) {

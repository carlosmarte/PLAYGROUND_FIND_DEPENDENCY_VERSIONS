 ...
 *
 *     cli.mjs (this REPL)  ->  GoVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `GoVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions github.com/stretchr/testify
 *     node cli.mjs run github.com/stretchr/testify --limit 5 --first-only -v
 *
 * Then, at the (go-versions) prompt:
 *     registry https://my-proxy.example.com
 *     versions github.com/stretchr/testify   # list what the proxy advertises
 *     find github.com/stretchr/testify        # fetch-test until the first that works
 *     test github.com/stretchr/testify        # fetch-test every version, write a JSON report
 *     test github.com/stretchr/testify 10     # fetch-test only the newest 10 versions
 *     verbose on            # stream full go output to debug fetch failures
...
import { GoVersionsSDK, GoVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "go-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(go-versions) ";

export class GoVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new GoVersionsSDK();
  ...
  _resolvePackage(arg) {
    ...
    if (!pkg) {
      console.log("No module set. Use 'package <name>' or pass one inline.");
    } else if (arg.trim()) {
  ...
  // registry [URL]  — set or show the proxy URL ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`proxy = ${this.sdk.effectiveIndexUrl() || "(go default)"}`);
  }

  // package [NAME]  — set or show the default module.
  do_package(arg) {
    arg = arg.trim();
  ...
  // venv [DIR]  — set or show the throwaway test module dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // go [VERSION|none]  — set or show the go version asserted in the test module.
  do_go(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.goVersion = arg;
      this.sdk.invalidateVenv(); // re-assert on next fetch-test
    }
    console.log(`go-version = ${this.cfg.goVersion}`);
  }

  // verbose [on|off]  — stream full go output so fetches are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  go        = ${this.cfg.goVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved go/TLS env vars (process.env or default).
  do_env() {
  ...
  // versions [PACKAGE]  — list versions the proxy advertises.
  do_versions(arg) {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof GoVersionsError) console.log(e.message);
      else throw e;
    }
  }

  // test [PACKAGE] [MAX]  — fetch-test versions (newest first), write report.
  async do_test(arg) {
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <module> [--proxy URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the throwaway test module dir"],
      ["go [VERSION|none]", "set or show the go version asserted in the test module"],
      ["verbose [on|off]", "stream full go output so fetches are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved go/TLS env vars"],
      ["versions [PACKAGE]", "list versions the proxy advertises"],
      ["find [PACKAGE]", "fetch-test until the first version that works"],
      ["test [PACKAGE] [MAX]", "fetch-test versions, write the JSON report"],
 ...
 *
 *     docker run <image> versions github.com/x/y      // -> REPL: `versions github.com/x/y`
 *     docker run <image> run github.com/x/y --limit 5 // -> batch via main.main
 *     docker run -it <image>                           // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new GoVersionsREPL();
  if (argv.length) {

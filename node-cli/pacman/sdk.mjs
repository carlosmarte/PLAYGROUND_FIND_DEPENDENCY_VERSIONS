 ...
 *
 *     cli.mjs   ->  PacmanVersionsSDK  ->  main.mjs
 *     external  ->  PacmanVersionsSDK  ->  main.mjs
 *
 ...
 * lazily provisions the isolated test cache dir, returns *structured* results
 * (`Report`), and is built to be:
 *
 *   * **extended** — subclass `PacmanVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("bash", { indexUrl: "https://archive.archlinux.org", limit: 5 });
 *     console.log(report.installable);        // ['5.2.026-1', ...]
 *
 *     const versions = await sdk.versions("bash"); // just list what the repository advertises
 ...
 *
 *     const s = new sdk.PacmanVersionsSDK({ indexUrl: "https://archive.archlinux.org", pacmanVersion: "6.1.0" });
 *     s.config.package = "bash";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.PacmanVersionsSDK {
 ...
 *
 *     await new sdk.PacmanVersionsSDK().run(["bash", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_PACMAN_VERSION = main.DEFAULT_PACMAN_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use pacman's default, no
// --repository; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class PacmanVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "PacmanVersionsError";
  }
}
 ...
 * `$PACMAN_REPOSITORY > $PACMAN_REGISTRY_URL > Arch archive` chain. An explicit
 * `null` means "use pacman's own default" (omit `--repository` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    pacmanVersion = DEFAULT_PACMAN_VERSION, // null/"none" => skip the version check
    limit = null,
    verbose = false, // stream full pacman output so installs are debuggable
    ...
    this.output = output;
    this.pacmanVersion = pacmanVersion;
    this.limit = limit;
...
export class PacmanVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._cachePath = null; // lazily provisioned test cache dir
  }
   ...
   * environment (`UNSET`), and `--pacman-version none` skips the host pacman
   * version check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const pacmanVersion = String(ns.pacmanVersion).toLowerCase() === "none" ? null : ns.pacmanVersion;
    return new PacmanVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The repo URL pacman will actually receive (`null` => pacman default). */
  effectiveIndexUrl() {
  ...
  /**
   * Drop the cached cache dir so the next op re-provisions it.
   *
   * Call after changing `config.venvDir` or `config.pacmanVersion`.
   */
  invalidateVenv() {
    this._cachePath = null;
  }

  /** Provision the test cache dir once and return its path. */
  ensurePip() {
    if (this._cachePath === null) {
      let pv = this.config.pacmanVersion;
      pv = pv === null || String(pv).toLowerCase() === "none" ? null : pv;
      this._cachePath = main.setupVenv(this.config.venvDir, pv, this.resolveEnv(), this.config.verbose);
    }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new PacmanVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new PacmanVersionsError(`no versions found for '${name}'`);
    }

    const cachePath = this.ensurePip();
    const results = await main.testInstallations(
      cachePath, name, indexUrl, versions, this.config.output,

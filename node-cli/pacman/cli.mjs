 ...
 *
 *     cli.mjs (this REPL)  ->  PacmanVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `PacmanVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions bash
 *     node cli.mjs run bash --limit 5 --first-only -v
 *
 * Then, at the (pacman-versions) prompt:
 *     registry https://archive.archlinux.org
 *     versions bash         # list what the repository advertises
 *     find bash             # install-test until the first version that works
 *     test bash             # install-test every version, write a JSON report
 *     test bash 10          # install-test only the newest 10 versions
 *     verbose on            # stream full pacman output to debug install failures
...
import { PacmanVersionsSDK, PacmanVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "pacman-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(pacman-versions) ";

export class PacmanVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new PacmanVersionsSDK();
  ...
  // registry [URL]  — set or show the pacman repo/archive URL ('none' to clear).
  do_registry(arg) {
    arg = arg.trim();
    if (arg) this.cfg.indexUrl = arg.toLowerCase() === "none" ? null : arg;
    console.log(`repository = ${this.sdk.effectiveIndexUrl() || "(pacman default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test cache dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // pacman [VERSION|none]  — set or show the pacman version expected in the test env.
  do_pacman(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.pacmanVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`pacman-version = ${this.cfg.pacmanVersion}`);
  }

  // verbose [on|off]  — stream full pacman output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output     = ${this.cfg.output}`);
    console.log(`  venv-dir   = ${this.cfg.venvDir}`);
    console.log(`  pacman     = ${this.cfg.pacmanVersion}`);
    console.log(`  verbose    = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved pacman/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof PacmanVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--repository URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test cache dir"],
      ["pacman [VERSION|none]", "set or show the pacman version expected in the test env"],
      ["verbose [on|off]", "stream full pacman output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved pacman/TLS env vars"],
      ["versions [PACKAGE]", "list versions the repository advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions bash         // -> REPL: `versions bash`
 *     docker run <image> run bash --limit 5    // -> batch via main.main
 *     docker run -it <image>                   // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new PacmanVersionsREPL();
  if (argv.length) {

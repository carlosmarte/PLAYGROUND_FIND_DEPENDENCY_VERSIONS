 ...
 *
 *     cli.mjs (this REPL)  ->  CabalVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `CabalVersionsSDK` directly.
 *
 ...
 *
 * Then, at the (cabal-versions) prompt:
 *     registry https://my-registry.example.com
 *     versions aeson        # list what the registry advertises
 *     find aeson            # fetch-test until the first version that works
 *     test aeson            # fetch-test every version, write a JSON report
 *     test aeson 10         # fetch-test only the newest 10 versions
 *     verbose on            # stream full cabal output to debug fetch failures
...
import { CabalVersionsSDK, CabalVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "cabal-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(cabal-versions) ";

export class CabalVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new CabalVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`index-url = ${this.sdk.effectiveIndexUrl() || "(cabal default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test fetch sandbox dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // cabal [VERSION|none]  — set or show the cabal version expected in the sandbox.
  do_cabal(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.cabalVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next fetch-test
    }
    console.log(`cabal-version = ${this.cfg.cabalVersion}`);
  }

  // verbose [on|off]  — stream full cabal output so fetches are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  cabal     = ${this.cfg.cabalVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved cabal/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof CabalVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test fetch sandbox dir"],
      ["cabal [VERSION|none]", "set or show the cabal version expected in the sandbox"],
      ["verbose [on|off]", "stream full cabal output so fetches are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved cabal/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "fetch-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new CabalVersionsREPL();
  if (argv.length) {

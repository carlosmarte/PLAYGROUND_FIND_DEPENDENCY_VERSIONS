 ...
 *
 *     cli.mjs   ->  CabalVersionsSDK  ->  main.mjs
 *     external  ->  CabalVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `CabalVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.find("aeson");       // stop at the first version that fetches
 *
 ...
 *
 *     const s = new sdk.CabalVersionsSDK({ indexUrl: "https://hackage.haskell.org", cabalVersion: "3.12.1.0" });
 *     s.config.package = "aeson";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.CabalVersionsSDK {
 ...
 *
 *     await new sdk.CabalVersionsSDK().run(["aeson", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_CABAL_VERSION = main.DEFAULT_CABAL_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use cabal's default, no
// --index-url; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class CabalVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "CabalVersionsError";
  }
}
 ...
 * `$CABAL_REGISTRY_URL > $HACKAGE_API_URL > hackage` chain. An explicit
 * `null` means "use cabal's own default" (omit `--index-url` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    cabalVersion = DEFAULT_CABAL_VERSION, // null/"none" => skip version check
    limit = null,
    verbose = false, // stream full cabal output so fetches are debuggable
...
export class CabalVersionsSDK {
   ...
   * environment (`UNSET`), and `--cabal-version none` skips the sandbox
   * version check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const cabalVersion = String(ns.cabalVersion).toLowerCase() === "none" ? null : ns.cabalVersion;
    return new CabalVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The index URL cabal will actually receive (`null` => cabal default). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.cabalVersion`.
   */
  invalidateVenv() {
    this._sandbox = null;
  }
  ...
  ensurePip() {
    if (this._sandbox === null) {
      let cv = this.config.cabalVersion;
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new CabalVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new CabalVersionsError(`no versions found for '${name}'`);
    }

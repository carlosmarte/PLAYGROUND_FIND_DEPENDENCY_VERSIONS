 ...
 *
 *     cli.mjs (this REPL)  ->  NpmVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `NpmVersionsSDK` directly.
 *
 ...
 *
 * Then, at the (npm-versions) prompt:
 ...
 *     verbose on            # stream full npm output to debug install failures
...
import { NpmVersionsSDK, NpmVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const PROMPT = "(npm-versions) ";

export class NpmVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new NpmVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`registry = ${this.sdk.effectiveIndexUrl() || "(npm default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test install-prefix dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // npm [VERSION|none]  — set or show the npm version pinned in the test prefix.
  do_npm(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.npmVersion = arg;
      this.sdk.invalidateVenv(); // re-pin on next install-test
    }
    console.log(`npm-version = ${this.cfg.npmVersion}`);
  }

  // verbose [on|off]  — stream full npm output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  npm       = ${this.cfg.npmVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved npm/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof NpmVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test install-prefix dir"],
      ["npm [VERSION|none]", "set or show the npm version pinned in the test prefix"],
      ["verbose [on|off]", "stream full npm output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved npm/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new NpmVersionsREPL();
  if (argv.length) {

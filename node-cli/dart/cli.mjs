 ...
 *
 *     cli.mjs (this REPL)  ->  DartVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `DartVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions http
 *     node cli.mjs run http --limit 5 --first-only -v
 *
...
import { DartVersionsSDK, DartVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const PROMPT = "(dart-versions) ";

export class DartVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new DartVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`hosted-url = ${this.sdk.effectiveIndexUrl() || "(pub default)"}`);
  }
  ...
  do_venv(arg) {
    ...
    console.log(`package-dir = ${this.cfg.venvDir}`);
  }
  ...
  do_dart(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.dartVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`dart-version = ${this.cfg.dartVersion}`);
  }
  ...
  do_show() {
    ...
    console.log(`  output      = ${this.cfg.output}`);
    console.log(`  package-dir = ${this.cfg.venvDir}`);
    console.log(`  dart        = ${this.cfg.dartVersion}`);
    console.log(`  verbose     = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved pub/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof DartVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  async do_run(arg) {
    ...
    if (!argv.length) {
      console.log("Usage: run <package> [--hosted-url URL] [--limit N] [--first-only]");
      return;
    }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test Dart package dir"],
      ["dart [VERSION|none]", "set or show the dart version expected in the test package"],
      ["verbose [on|off]", "stream full pub output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved pub/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
...
export async function main(argv = null) {
  ...
  const repl = new DartVersionsREPL();
  if (argv.length) {

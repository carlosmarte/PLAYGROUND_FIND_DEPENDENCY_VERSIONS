 ...
 *
 *     cli.mjs   ->  DartVersionsSDK  ->  main.mjs
 *     external  ->  DartVersionsSDK  ->  main.mjs
 *
 ...
 * lazily provisions the isolated test Dart package, returns *structured* results
 * (`Report`), and is built to be:
 *
 *   * **extended** — subclass `DartVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("http", { indexUrl: "https://pub.dev", limit: 5 });
 *     console.log(report.installable);        // ['1.2.2', ...]
 *
 *     const versions = await sdk.versions("http"); // just list what the registry advertises
 *     const report = await sdk.find("http");       // stop at the first version that installs
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("http");
 ...
 *
 *     sdk.versionsOutput("http");        // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.DartVersionsSDK({ indexUrl: "https://pub.dev", dartVersion: "3.5.4" });
 *     s.config.package = "http";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.DartVersionsSDK {
 ...
 *
 *     await new sdk.DartVersionsSDK().run(["http", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_DART_VERSION = main.DEFAULT_DART_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use pub's default hosted URL, no
// override; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class DartVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "DartVersionsError";
  }
}
 ...
 * `$PUB_HOSTED_URL > $DART_REGISTRY_URL > pub.dev` chain. An explicit
 * `null` means "use pub's own default hosted URL" (omit the override).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    dartVersion = DEFAULT_DART_VERSION, // null/"none" => skip the check
    limit = null,
    verbose = false, // stream full pub output so installs are debuggable
...
export class DartVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._pkgDir = null; // lazily provisioned test-package directory
  }
   ...
   *
   * Mirrors the CLI exactly: an absent `--hosted-url` resolves from the
   * environment (`UNSET`), and `--dart-version none` skips the dart version
   * confirmation step.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const dartVersion = String(ns.dartVersion).toLowerCase() === "none" ? null : ns.dartVersion;
    return new DartVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The hosted URL pub will actually receive (`null` => pub default). */
  effectiveIndexUrl() {
  ...
  /**
   * Drop the cached test package so the next op re-provisions it.
   *
   * Call after changing `config.venvDir` or `config.dartVersion`.
   */
  invalidateVenv() {
    this._pkgDir = null;
  }

  /** Provision the test Dart package once and return its directory. */
  ensurePip() {
    if (this._pkgDir === null) {
      let dv = this.config.dartVersion;
      dv = dv === null || String(dv).toLowerCase() === "none" ? null : dv;
      this._pkgDir = main.setupVenv(this.config.venvDir, dv, this.resolveEnv(), this.config.verbose);
    }
    return this._pkgDir;
  }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new DartVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new DartVersionsError(`no versions found for '${name}'`);
    }

    const pkgDir = this.ensurePip();
    const results = await main.testInstallations(
      pkgDir, name, indexUrl, versions, this.config.output,

 ...
 *
 *     cli.mjs (this REPL)  ->  ChocoVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `ChocoVersionsSDK` directly.
 *
 ...
 * exits:
 *     node cli.mjs versions git
 *     node cli.mjs run git --limit 5 --first-only -v
 *
 * Then, at the (choco-versions) prompt:
 *     registry https://community.chocolatey.org/api/v2/
 *     versions git   # list what the registry advertises
 *     find git       # install-test until the first version that works
 *     test git       # install-test every version, write a JSON report
 *     test git 10    # install-test only the newest 10 versions
 *     verbose on     # stream full choco output to debug install failures
...
import { ChocoVersionsSDK, ChocoVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "choco-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(choco-versions) ";

export class ChocoVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new ChocoVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`source = ${this.sdk.effectiveIndexUrl() || "(choco default)"}`);
  }
  ...
  // choco [VERSION|none]  — set or show the choco version the test env checks.
  do_choco(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.chocoVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`choco-version = ${this.cfg.chocoVersion}`);
  }

  // verbose [on|off]  — stream full choco output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  choco     = ${this.cfg.chocoVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved choco/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof ChocoVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["verbose [on|off]", "stream full choco output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved choco/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
 ...
 *
 *     docker run <image> versions git       // -> REPL: `versions ...`
 *     docker run <image> run git --limit 5  // -> batch via main.main
 *     docker run -it <image>                 // -> interactive REPL
 */
export async function main(argv = null) {
  argv = argv === null ? process.argv.slice(2) : argv;
  const repl = new ChocoVersionsREPL();
  if (argv.length) {

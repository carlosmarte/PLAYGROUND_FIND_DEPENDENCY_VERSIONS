 ...
 *
 *     cli.mjs   ->  ChocoVersionsSDK  ->  main.mjs
 *     external  ->  ChocoVersionsSDK  ->  main.mjs
 *
 ...
 *
 *   * **extended** — subclass `ChocoVersionsSDK` and override the `beforeProbe`
 ...
 *     const report = await sdk.test("git", { source: "https://community.chocolatey.org/api/v2/", limit: 5 });
 *     console.log(report.installable);        // ['2.47.1', ...]
 *
 *     const versions = await sdk.versions("git"); // just list what the registry advertises
 *     const report = await sdk.find("git");       // stop at the first version that installs
 *
 * Structured output (call from any consuming script — no console scraping):
 *
 *     const report = await sdk.test("git");
 ...
 *
 *     sdk.versionsOutput("git");          // { package, indexUrl, count, versions }
 *
 * Object form (hold one per session, mutate `.config` freely):
 *
 *     const s = new sdk.ChocoVersionsSDK({ source: "https://community.chocolatey.org/api/v2/", chocoVersion: "2.3.0" });
 *     s.config.package = "git";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.ChocoVersionsSDK {
 ...
 *
 *     await new sdk.ChocoVersionsSDK().run(["git", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_CHOCO_VERSION = main.DEFAULT_CHOCO_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use choco's default, no
// --source; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class ChocoVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "ChocoVersionsError";
  }
}
 ...
 * `$CHOCO_SOURCE > $CHOCO_REGISTRY_URL > community.chocolatey.org` chain. An
 * explicit `null` means "use choco's own default" (omit `--source` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    chocoVersion = DEFAULT_CHOCO_VERSION, // null/"none" => skip toolchain check
    limit = null,
    verbose = false, // stream full choco output so installs are debuggable
    ...
    this.output = output;
    this.chocoVersion = chocoVersion;
    this.limit = limit;
...
export class ChocoVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides. Accept
    // `source` as a friendly alias for the `indexUrl` field.
    if (config && !(config instanceof Config)) {
      config = { ...config };
      if ("source" in config) {
        config.indexUrl = config.source;
   ...
   * (`UNSET`), and `--choco-version none` skips the toolchain check.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const chocoVersion = String(ns.chocoVersion).toLowerCase() === "none" ? null : ns.chocoVersion;
    return new ChocoVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The source URL choco will actually receive (`null` => its default). */
  effectiveIndexUrl() {
   ...
   *
   * Call after changing `config.venvDir` or `config.chocoVersion`.
   */
  invalidateVenv() {
    this._venvDir = null;
  }
  ...
  ensurePip() {
    if (this._venvDir === null) {
      let cv = this.config.chocoVersion;
      cv = cv === null || String(cv).toLowerCase() === "none" ? null : cv;
      this._venvDir = main.setupVenv(this.config.venvDir, cv, this.resolveEnv(), this.config.verbose);
    }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new ChocoVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new ChocoVersionsError(`no versions found for '${name}'`);
    }

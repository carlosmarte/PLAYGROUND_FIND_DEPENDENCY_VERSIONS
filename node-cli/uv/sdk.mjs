 ...
 *
 *     cli.mjs   ->  UvVersionsSDK  ->  main.mjs
 *     external  ->  UvVersionsSDK  ->  main.mjs
 *
 ...
 * lazily provisions the isolated uv venv sandbox, returns *structured* results
 * (`Report`), and is built to be:
 *
 *   * **extended** — subclass `UvVersionsSDK` and override the `beforeProbe`
 ...
 *
 *     const s = new sdk.UvVersionsSDK({ indexUrl: "https://reg/simple", uvVersion: "0.4.18" });
 *     s.config.package = "numpy";
 *     await s.test({ limit: 10 });
 *
 * Extension example:
 *
 *     class QuietSDK extends sdk.UvVersionsSDK {
 ...
 *
 *     await new sdk.UvVersionsSDK().run(["numpy", "--limit", "5", "--first-only"]);
 */
...
export const DEFAULT_UV_VERSION = main.DEFAULT_UV_VERSION;
export const ENV_DEFAULTS = main.ENV_DEFAULTS;

// Sentinel for "argument not supplied" — distinct from an explicit `null`,
// which is itself meaningful (indexUrl=null => use uv's default, no
// --index-url; limit=null => test every version).
export const UNSET = Symbol("UNSET");

export class UvVersionsError extends Error {
  constructor(message) {
    super(message);
    this.name = "UvVersionsError";
  }
}
 ...
 * `$UV_INDEX_URL > $PYTHON_REGISTRY_URL > pypi.org` chain. An explicit `null`
 * means "use uv's own default" (omit `--index-url` entirely).
 */
export class Config {
  constructor({
    package: pkg = null,
    ...
    uvVersion = DEFAULT_UV_VERSION, // null/"none" => keep installed uv
    limit = null,
    verbose = false, // stream full uv output so installs are debuggable
...
export class UvVersionsSDK {
  constructor(config = null) {
    // Accept either a Config instance or a plain object of overrides.
    this.config = config instanceof Config ? config : new Config(config || {});
    this._pyPath = null; // lazily provisioned test-venv python
  }
   ...
   * environment (`UNSET`), and `--uv-version none` keeps whatever uv is on PATH.
   */
  static fromArgv(argv) {
    const ns = main.parseArgs(argv);
    const uvVersion = String(ns.uvVersion).toLowerCase() === "none" ? null : ns.uvVersion;
    return new UvVersionsSDK(new Config({
      package: ns.package,
  ...
  /** The index URL uv will actually receive (`null` => uv default). */
  effectiveIndexUrl() {
  ...
  /**
   * Drop the cached venv python so the next op re-provisions it.
   *
   * Call after changing `config.venvDir` or `config.uvVersion`.
   */
  invalidateVenv() {
    this._pyPath = null;
  }

  /** Provision the uv venv once and return its python path. */
  ensurePip() {
    if (this._pyPath === null) {
      let uv = this.config.uvVersion;
      uv = uv === null || String(uv).toLowerCase() === "none" ? null : uv;
      this._pyPath = main.setupVenv(this.config.venvDir, uv, this.resolveEnv(), this.config.verbose);
    }
    return this._pyPath;
  }
  ...
  _requirePackage(pkg) {
    ...
    if (!name) {
      throw new UvVersionsError("no package set; pass one or set config.package");
    }
  ...
  async _probe(pkg, limit, firstOnly) {
    ...
    if (!versions.length) {
      throw new UvVersionsError(`no versions found for '${name}'`);
    }

    const pyPath = this.ensurePip();
    const results = await main.testInstallations(
      pyPath, name, indexUrl, versions, this.config.output,

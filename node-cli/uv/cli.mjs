 ...
 *
 *     cli.mjs (this REPL)  ->  UvVersionsSDK  ->  main.mjs
 *
 ...
 * external caller can do by driving the same `UvVersionsSDK` directly.
 *
 ...
 *
 * Then, at the (uv-versions) prompt:
 ...
 *     verbose on            # stream full uv output to debug install failures
...
import { UvVersionsSDK, UvVersionsError, ENV_DEFAULTS } from "./sdk.mjs";
...
const INTRO =
  "uv-versions interactive shell. Type 'help' or '?' for commands, " +
...
const PROMPT = "(uv-versions) ";

export class UvVersionsREPL {
  constructor(client = null) {
    ...
    this.sdk = client || new UvVersionsSDK();
  ...
  do_registry(arg) {
    ...
    console.log(`index-url = ${this.sdk.effectiveIndexUrl() || "(uv default)"}`);
  }
  ...
  // venv [DIR]  — set or show the test uv venv dir (resets the env).
  do_venv(arg) {
    arg = arg.trim();
    if (arg) {
  ...
  // uv [VERSION|none]  — set or show the uv version expected for the test venv.
  do_uv(arg) {
    arg = arg.trim();
    if (arg) {
      this.cfg.uvVersion = arg;
      this.sdk.invalidateVenv(); // re-check on next install-test
    }
    console.log(`uv-version = ${this.cfg.uvVersion}`);
  }

  // verbose [on|off]  — stream full uv output so installs are debuggable.
  do_verbose(arg) {
    arg = arg.trim().toLowerCase();
  ...
  do_show() {
    ...
    console.log(`  output    = ${this.cfg.output}`);
    console.log(`  venv-dir  = ${this.cfg.venvDir}`);
    console.log(`  uv        = ${this.cfg.uvVersion}`);
    console.log(`  verbose   = ${this.cfg.verbose ? "on" : "off"}`);
  }

  // env  — show the resolved uv/TLS env vars (process.env or default).
  do_env() {
  ...
  async do_find(arg) {
    ...
    try {
      ...
      if (e instanceof UvVersionsError) console.log(e.message);
      else throw e;
    }
  }
  ...
  do_help() {
    const docs = [
      ...
      ["limit [N|none]", "only probe the newest N versions ('none' = all)"],
      ["output [PATH]", "set or show the JSON report path"],
      ["venv [DIR]", "set or show the test uv venv dir"],
      ["uv [VERSION|none]", "set or show the uv version expected for the test venv"],
      ["verbose [on|off]", "stream full uv output so installs are debuggable"],
      ["show", "print the current session settings"],
      ["env", "show the resolved uv/TLS env vars"],
      ["versions [PACKAGE]", "list versions the registry advertises"],
      ["find [PACKAGE]", "install-test until the first version that works"],
